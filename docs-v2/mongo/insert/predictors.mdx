...
description: Predictors are the machine learning models that enable us to forecast future data based on the available data.
---

By using the `db.predictors.insert()` method, we create and train predictors in Mongo.
...
After running the `db.predictors.insert()` method, execute the
`db.predictors.find()` method from the `mindsdb.predictors` collection to check
...
[`mindsdb.predictors`](/mongo/collection-structure/#the-predictors-collection)
