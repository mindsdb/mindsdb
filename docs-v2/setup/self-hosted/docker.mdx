...
By default, Docker for Mac allocates 2 GB of RAM, which is insufficient for deploying MindsDB with
Docker. We recommend increasing the default RAM limit to 4 GB. Please refer to
the [Docker Desktop for Mac users manual](https://docs.docker.com/desktop/mac/#resources)
...
- `#!bash -p 47334:47334` publishes port 47334 to access MindsDB GUI and HTTP
  API
...
complete, please add the `#!bash MKL_SERVICE_FORCE_INTEL` environment variable,
as below.
