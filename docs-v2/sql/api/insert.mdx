...
The ` INSERT INTO` statement is used to insert data into a table. This inserted
data comes from the subselect statement. It is commonly used to input
...
It performs a subselect ` (SELECT ...)` and gets data from there. Then, this
data is inserted into a table of integration ` [integration_name]` using
` INSERT INTO TABLE [table_name]` statement.
...
In this example, we want to input the predictions into a table ` int1.tbl1`.
