...
The `SELECT` statement is used to get a predictions from the model table. The
data is not persistent and is returned on the fly as a result-set. The basic
syntax for selecting from the model is:
...
Ensure to not use any quotations for numerical values and singular quotes for
strings.
...
| `WHERE [column]=[value] AND ...` | `WHERE` clause used to pass the input data to make the prediction                                                           |

## Example

The following SQL statement selects a `rental_price` prediction from the
`home_rentals_model` for a property that has the attributes named after the
`WHERE` expression:
