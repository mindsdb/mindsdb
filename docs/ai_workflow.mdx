...
MindsDB integrates numerous data sources, applications, and AI frameworks. You can choose out of a wide variety of integrations and create your custom AI Workflow, following the steps outlined in the **AI Workflow** documentation section.
...
You can create a connection between your data source and MindsDB using the `CREATE DATABASE` statement in SQL, or corresponding one for other dev environments.
...
MindsDB integrates numerous AI frameworks. Before you can create an AI model, it is required to create an ML engine based on the AI framework of your choice. Oftentimes, creation of an ML engine requires users to provide parameters such as API keys.
...
Once the ML engine is created succesfully, you can proceed to creating an AI model, as shown in the following steps.

### Step 4. Create Projects

In MindsDB, all objects (including models, views, jobs, etc.) are stored within projects. You can use the default `mindsdb` project or create your own to store usecase-specific objects in different projects.
...
- You automate this process by creating a job that does the following:
    ...
    - Next, the job queries the AI model joined with the Binance data to get the latest rela-time forecasts, and subsequently, inserts them to the Slack workspace.
