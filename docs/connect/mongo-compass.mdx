...
<Tabs>
  ...
  <Tab title= "Connecting to Local MindsDB Installation">
    ...
    First, create a new connection in MongoDB Compass by clicking the `New Connection` button from the left navigation panel.

    The host value is `127.0.0.1` and the port value is `47336`. Input the value in the `Host` field, as below.
  ...
  <Tab title="Connecting to MindsDB Cloud">
    ...
    The host value is `cloud.mindsdb.com` and the port value is `27017`. Input the value in the `Host` field, as below.
