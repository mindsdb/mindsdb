...
<Info>
Before installing MindsDB from source, ensure that you use one of the following Python versions: `3.10.x`, `3.11.x`, `3.12.x`, or `3.13.x`.
</Info>
...
3. Create a virtual environment:
   ...
   - Windows (also works with the `py` launcher):
...
<Tip>
Windows PowerShell may block script execution. If activation fails with a policy error, run PowerShell "as Administrator" once and execute:
