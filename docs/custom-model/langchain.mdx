...
## How to Bring the LangChain Model to MindsDB

We use the [`CREATE MODEL`](/sql/create/model) statement to bring the LangChain models to MindsDB.
...
Here, we create the `tool_based_agent` model using the LangChain engine, as defined in the `engine` parameter. This model answers users' questions in a helpful way, as defined in the `prompt_template` parameter.

There are three different tools utilized by this model:

* **MindsDB** is an internal MindsDB executor.
* **Metadata** fetches the metadata information for the available tables.
* **Write** creates a response in a string format.

Each tool exposes the internal MindsDB executor in a different way to perform its tasks.
...
WHERE input = 'Could you describe the `files.house_sales` table please?'
USING
    verbose = True,
    tools = [],
    max_iterations = 10;
...
| input                                                    | completion                                                                                                                                    |
+----------------------------------------------------------+-----------------------------------------------------------------------------------------------------------------------------------------------+
| Could you describe the `files.house_sales` table please? | The `files.house_sales` table has 347 rows and contains columns for `saledate`, `MA`, `type`, and `bedrooms`, all of which are of type `str`. |
+----------------------------------------------------------+-----------------------------------------------------------------------------------------------------------------------------------------------+
```

To get information about the `files.house_sales` table, the agent uses the Metadata tool. Then the Write tool prepares the response.
...
This query returns the value of 1.6, which is then used by the Write tool to create an answer.
...
The model created the `INSERT INTO` statement and executed it on the `local_database` database.
