...
USING
  ...
  prompt_template = 'task description';         -- DEFINES THE TASK TO BE SOLVED BY OPENAI
```
...
| `engine`         | An ML engine based on which the model is created.                                                   |
...
<Note>
In the `prompt_template`, we use a placeholder for a text value that comes from the `review` column, that is, `text:{{review}}`.
</Note>
