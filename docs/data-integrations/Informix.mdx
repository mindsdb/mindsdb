...
* `logging_enabled`: Is loging is enabled or not. Default is True
...
```
This integration uses IfxPy it is in development stage there it can be install using pip install IfxPy.But it doesn't work for higher version of python, therefore you have to build it from source.
