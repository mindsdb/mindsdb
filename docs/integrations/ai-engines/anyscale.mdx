...
USING
      ...
      prompt = 'system prompt',  -- optional, system prompt for the model
      ...
      prompt_template = 'input message to the model here', -- optional, base template with placeholders used to provide input to the model 
...
<AccordionGroup>
  ...
  <Accordion title="model_name">
    ...
    You can find [all available models here](https://docs.anyscale.com/1.0.0/endpoints/overview/#supported-models).
  </Accordion>
...
`files.unrelated_questions` is a simple CSV file containing a `question` column with simple (unrelated) questions that has been uploaded to MindsDB, while `files.related_questions` is a similar file containing related questions. These files are used in the examples given below to provide multiple inputs to the models created. It is possible to use any other supported data source in the same manner.
</Note>
