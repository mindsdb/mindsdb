...
---------------------------------------------------------------------------------------
TODO when this is merged https://github.com/mindsdb/mindsdb/pull/8765/files#diff-0fc65b238a4995425779a3a697ab8a73048fdbe91b9abef89082c2dbee3a93a2
...
3. Obtain the API key for one of OpenAI or Anyscale or LiteLLM required to deploy and use LangChain models within MindsDB. Follow the [instructions for obtaining the API key](<link>).
...
FROM langchain
[USING
      ..._api_key = 'api-key-value'];
```

<any additional info here>

Create a model using `langchain_engine` as an engine.

```sql
CREATE MODEL <ai-framework-name_model>
[FROM integration
         (SELECT * FROM table)]
...
[HORIZON <int>]
[USING
      engine = '<ai-framework-name_engine>',  -- engine name as created via CREATE ML_ENGINE
      <other params specific to an ai engine>];
```
...
The following usage examples utilize `<ai-framework-name_engine>` to create a model with the `CREATE MODEL` statement.

<short description of a usage example>
...
Follow [this tutorial](<link to a tutorial>) to see more use case examples.
<if no tutorial exists, then direct users to the Use Cases section: Go to the **Use Cases** section to see more examples.>
</Tip>
