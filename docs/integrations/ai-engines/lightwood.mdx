...
<Note>
The `confidence` score is produced by the conformal prediction module and is statistically valid. On the other hand, the `probability_class` comes directly from the model logits, which may be uncalibrated. Therefore, this `confidence` score is not right, on average - it may be optimistic or pessimistic, for instance.
</Note>
