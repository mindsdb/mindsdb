...
This documentation describes the integration of MindsDB with [Minds Endpoint](https://docs.mdb.ai/), a cloud service that simplifies the way developers interact with cutting-edge LLMs through a universal API and provides Minds, which are AI systems with built-in expertise designed to help AI agents accomplish tasks.
...
3. Obtain the Minds Endpoint API key required to deploy and use Minds Endpoint models within MindsDB. Follow the [instructions for obtaining the API key](https://docs.mdb.ai/docs/authentication).
...
Create a model using `minds_endpoint_engine` as an engine.

```sql
CREATE MODEL minds_endpoint_model
PREDICT answer
USING
      engine = 'minds_endpoint_engine',   -- engine name as created via CREATE ML_ENGINE
...
The following usage examples utilize `minds_endpoint_engine` to create a model with the `CREATE MODEL` statement.
