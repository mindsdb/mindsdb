...
<Tip>
In order to use the `Binance` Handler, there is no need to pass anything.
</Tip>
...
### Select Data
By default, aggregate data (klines) from the latest 1000 trading intervals with a length of 1m each will be returned.
...
To create a time series model using 10000 trading intervals in the past with duration 1m.
...
<Note>
For more accuracy, the limit can be higher to a higher value (e.g. 100,000)
</Note>
