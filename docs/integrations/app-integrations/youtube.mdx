...
Establish a connection to YouTube from MindsDB by executing the following SQL command:
...
To do that, click on the `Add` button, choose `New Datasource`, search for `YouTube`, and follow the instructions in the form.
</Tip>
