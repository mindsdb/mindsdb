...
<Tabs>
    <Tab title="Template">
    ```sql
    CREATE DATABASE gcs_datasource          --- display name for the database
    WITH ENGINE = 'gcs',                    --- name of the MindsDB handler
    PARAMETERS = {
      "gcs_access_key_id": ' ',             --- the Google HMAC access key id
      "gcs_secret_access_key": ' ',         --- the Google HMAC secret access key
      "service_account_keys": ' ',          --- the path to service account json file
      "bucket": ' ',                        --- name of the GCS bucket
      "prefix": ' ',                        --- a string to filter you objects (files)
      "file_type": ' '                      --- the file format of your data
    };
    ```
    </Tab>
    <Tab title="Example">
...
<Info>
Check out the Google Cloud Storage data handler details [here](/data-integrations/google-cloud-storage).
</Info>
