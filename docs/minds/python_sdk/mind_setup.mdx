...
In the below sample code, we connect to our sample database ([see full details here](/docs/sample_data_sources)) and create a Mind. As a **prerequisite**, install the latest version of Minds SDK by running `pip install minds_sdk`.
...
- `engine` is the engine of the data source to be connected. [See all supported data sources here](/docs/data_sources).
- `connection_data` stores the connection parameters that allow Minds to access the data source. [See all supported data sources here](/docs/data_sources).
...
- `datasources` is a list of configurations for data sources, such as databases, you can connect to your Mind. [See all supported data sources here](/docs/data_sources).

**Now that you created the Mind, let's start the conversation with your data!**

- [Completions API](/docs/data-mind-completions) – suits for simple query-response interactions, yet is stateless.
- [Assistants API](/docs/data-mind-assistants) – for context persistence and advanced features like dynamic data retrieval or code execution.
