...
Agents can be assigned different model available from [LangChain integration](/integrations/ai-engines/langchain).

The `provider` parameter defined the provider of the model. Its available values include `openai`, `anthropic`, `anyscale`, `ollama`, `litellm`, and `mindsdb`.
