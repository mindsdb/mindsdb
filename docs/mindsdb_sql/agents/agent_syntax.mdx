...
* `api_key`
The API key is required to access the model. Users can provide it either in this `api_key` parameter, or using [environment variables](/mindsdb_sql/functions/from_env).
...
* `knowledge_bases` stores the list of [knowledge bases](/mindsdb_sql/knowledge-bases) to be used by the agent.
