...
USING
     ...
     storage = my_vector_store, -- optional, default ChromaDB
...
* `embedding_model` stores specification of the embedding model to be used.
      * `model_name` defines the embedding model as specified by the provider. Users can choose one of the [OpenAI embedding models](https://platform.openai.com/docs/guides/embeddings/embedding-models#embedding-models).
      ...
      * `base_url` specifies the base URL of the model provider. It defaults to `https://api.openai.com/v1`.

* `rerank_model` stores specification of the reranking model to be used.
      * `model_name` defines the reranking model as specified by the provider. Users can choose one of the [OpenAI chat models](https://platform.openai.com/docs/models).
      * `api_key` stores your OpenAI API key. Alternatively, you can use the `OPENAI_API_KEY` environment variable to store your OpenAI API key.
      * `base_url` specifies the base URL of the model provider. It defaults to `https://api.openai.com/v1`.

* `storage` is an optional parameter. It assigns the name of the vector store connected to MindsDB, either [PGVector](/integrations/vector-db-integrations/pgvector) or [ChromaDB](/integrations/vector-db-integrations/chromadb). If not provided, the system uses the default ChromaDB vector database.
