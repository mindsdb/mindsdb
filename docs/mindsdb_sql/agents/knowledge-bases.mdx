...
USING
    model = <embedding_model_name>, -- optional
...
* `model` is an optional parameter. It assigns a pre-existing [embedding model for generating vector embeddings](integrations/ai-engines/langchain_embedding).
...
<Note>
Here are the conditions supported by the available verctor stores:
