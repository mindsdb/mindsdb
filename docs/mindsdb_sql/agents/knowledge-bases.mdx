...
Internally, knowledge bases use a vector store and an embedding model. By default, it uses the ChromaDB vector store and the OpenAI embedding model, which requires an OpenAI API key. You can define a vector store and an embedding model as in the examples below.
...
- `model`: This is an embedding model created within MindsDB with `CREATE MODEL embedding_model`.
...
- `metadata_columns`: The list of column names that will store metadata in the knowledge base. If not set, there are no metadata columns.
- `content_columns`: The list of column names that will store actual content in the knowledge base. If not set, all columns are considered to be content columns.
- `id_column`: The column name that will store unique identitifiers in the knowledge base. It defaults to `id`.
