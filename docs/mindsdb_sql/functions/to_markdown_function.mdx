...
<Info>
At the moment, the `TO_MARKDOWN()` function only supports PDF documents. The documents can be provided from URLs, file storage, or Amazon S3 storage.
</Info>
...
<AccordionGroup>
  ...
  3. Run the above query, passing the public URL of the file as an argument.
