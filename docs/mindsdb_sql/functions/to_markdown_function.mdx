...
The `TO_MARKDOWN()` function requires a large language model, which can be defined in the [MindsDB configuration under `default_llm`](/setup/custom-config) or via environment variables as below.
