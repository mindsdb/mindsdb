...
* `api_key`
It is an optional parameter, which defaults to the value stored in the `OPENAI_API_KEY` environment variable. It stores your OpenAI API key. Note that if you do not provide the `api_key` parameter when creating the knowledge base, you require to have the `OPENAI_API_KEY` environment variable set.
...
The reranking model is a required component of the knowledge base. It stores specifications of the reranking model to be used.
...
* `threads` defines threads for running partitions. Note that if the [ML task queue](/setup/custom-config#overview-of-config-parameters) is enabled, threads are used automatically. The available values for include `int` (for example, `10`), which defines the number of threads to be used, or `boolean` (for example, `true` or `false`), which defines whether to enable or disable threads.

* `track_column` defines the column used for sorting data before partitioning.

* `error` defines the error processing options. The available values include `raise` or `skip`. It defaults to `raise` if not provided.
...
<Note>
When inserting into a knowledge base where the `content_columns` parameter was not specified, the column storing content must be aliased `AS content`.
