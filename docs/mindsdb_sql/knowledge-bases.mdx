...
This parameter stores the name of the table that comes from one of the data sources connected to MindsDB. For example, `test_table = my_datasource.my_test_table` defines a table named `my_test_table` from the data source named `my_datasource`.

This test table stores test data in form of questions and answers.

Users can provide their own test data or have the test data generated by the `EVALUATE KNOWLEDGE_BASE` command, which is performed when setting the `generate_data` parameter.

### `version`

This parameter defines the version of the evaluator.
