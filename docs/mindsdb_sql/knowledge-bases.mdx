...
* `id_column` is the column name that uniquely identifies each source data row.

<Info>
The `id_column` parameter is mandatory and the knowledge base uses it to identify rows from the source data.

Note that if the source data row is chunked into multiple chunks, then these rows in the knowledge base have the same ID value that identifies chunks from one source data row.
</Info>
