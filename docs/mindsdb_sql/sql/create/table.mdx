...
<Note>
Note that the `integration_name` connection must be created with the [`CREATE DATABASE`](/mindsdb_sql/sql/create/database) statement. The connected data source must provide the write access to create a table.
</Note>
