...
A machine learning (ML) model is a program trained using the available data in order to learn how to recognize patterns and behaviors and make decisions that lead to predicting future data.
...
Features define the columns or variables that an ML model uses as input data to search for patterns and predict the target variable.

### Target

A target is a column or variable that an ML model predicts based on the information fetched out of the features.
...
The ML model takes the entire training dataset as input. It learns the patterns and relationships between feature variables and correlates them with the target variable values.

Once the training process is complete, one can move on to the validation phase.

### Validation Dataset

The validation dataset is used during the validation phase of an ML model. It contains both feature variables and a target variable, like the training dataset. But as its name indicates, it is used to validate the predictions made by an ML model.
...
## How an ML Model is Created?

In MindsDB, we use the [CREATE MODEL](/sql/create/predictor/) statement to create, train, and validate a model.
...
The `CREATE MODEL` statement performs validation of the model.
...
<Note>
Please note that MindsDB calculates the model's accuracy while running the `CREATE MODEL` statement.

The `CREATE MODEL` statement does the following:
...
Time series models fall under the regression or classification category. But what's distinct about them is that we order data by date, time, or any value defining sequential order of events.
...
MindsDB uses the Lightwood ML engine by default. The so-called mixers are used to create models by detecting patterns and behaviors of data. Here are [the mixers used by Lightwood](https://lightwood.io/mixer.html).
...
- And [here](https://github.com/mindsdb/lightwood/blob/9a48d5523d78111c77024e1983d281f37a9782ea/lightwood/api/json_ai.py#L353,L358), we choose the best of submodels to be used to create, train, and validate our ML model.
...
<Note>
MindsDB may use one or multiple mixers while preparing a model. Depending on the model type and the data type of the target variable, one mixer or a set of mixers is chosen to create, train, and validate an ML model.
</Note>

The three cases above describe how MindsDB chooses the mixer candidates and stores them in the `submodels` array.

By default, after collecting all relevant mixers in the `submodels` array, MindsDB uses the [BestOf](https://lightwood.io/ensemble.html#ensemble.BestOf) ensemble to [choose one of these mixers to fit the model](https://github.com/mindsdb/lightwood/blob/9a48d5523d78111c77024e1983d281f37a9782ea/lightwood/api/json_ai.py#L353,L358).

But you can always use a different ensemble that allows using multiple mixers per model, such as the [MeanEnsemble](https://lightwood.io/ensemble.html#ensemble.MeanEnsemble), [ModeEnsemble](https://lightwood.io/ensemble.html#ensemble.ModeEnsemble), [StackedEnsemble](https://lightwood.io/ensemble.html#ensemble.StackedEnsemble), [TsStackedEnsemble](https://lightwood.io/ensemble.html#ensemble.TsStackedEnsemble), or [WeightedMeanEnsemble](https://lightwood.io/ensemble.html#ensemble.WeightedMeanEnsemble) ensemble type.
