...
We have a sample Mongo database that you can connect to your MindsDB Cloud account by running this command:

```sql
CREATE DATABASE mongo_test_db
WITH ENGINE = 'mongodb',
PARAMETERS = {'host': 'mongodb+srv://admin:201287aA@cluster0.myfdu.mongodb.net/admin?authSource=admin&replicaSet=atlas-5koz1i-shard-0&readPreference=primary&appname=MongoDB%20Compass&ssl=true',
              'database': 'test_data'};
```
...
Now, we are ready to create a model. The following commands should be executed from Mongo database.

First, we switch to the `mindsdb` project:
...
Let's create a Hugging Face model.
