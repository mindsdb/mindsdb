...
- Summarization
<Tip>
The MindsDB's Hugging Face engine is extensible and we are actively working to add more tasks and models.
...
<AccordionGroup>
    <Accordion title="Text-classification" defaultOpen="true">
        Task of assigning a label to a text. For example, you can use it to classify a movie review as positive or negative.
        ##### Supported Models
    ...
    <Accordion title="Zero-shot-classification" defaultOpen="true">
        Task of assigning a label to a text without training on the labels.
    ...
    <Accordion title="Translation" defaultOpen="true">
        Task of translating a text from one language to another. For example, you can use it to translate a text from English to French.
        ##### Supported Models
    ...
    <Accordion title="Summarization" defaultOpen="true">
        Task of summarizing a text. For example, you can use it to summarize an abstract into a title.
        ##### Supported Models
...
The Hugging Face's models are used to perform these tasks.
...
We use the [`CREATE MODEL`](/sql/create/predictor) statement to bring Hugging Face's models to MindsDB.
...
| `task`           | Optional, relative to Hugging Face task tag.                                                     |
| `model_name`     | Model name under the Hugging Face model hub                                                      |
| `input_column`   | Name of the column that has the input data, specially important for batch predcitions using JOIN |
| `labels`         | Dependending on the model, usually used for Zero-shot-classification models                      |

### Example

Let's go through an Spam classification example to understand better how
to link Hugging Face's models and bring them to MindsDB as AI tables.

<Note>
    **Using Local Installation of MindsDB**
    Please note that if you use local installation of MindsDB, instead of MindsDB Cloud, you should install `transformers==4.21.0` to be able to use the Hugging Face's models.
...
For the a full library of supported examples please go [here](/nlp/nlp-extended-examples)