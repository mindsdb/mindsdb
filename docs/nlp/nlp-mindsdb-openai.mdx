...
MindsDB lets you fine-tune the OpenAI models with your own data using the `ADJUST` statement. Let's go over an example.

Here, we create a model using the OpenAI engine, and then, fine-tune it using the `ADJUST` statement.
