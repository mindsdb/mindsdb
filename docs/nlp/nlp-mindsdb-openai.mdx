...
We use the [`CREATE ML_ENGINE`](/sql/create/ml-engine) and [`CREATE MODEL`](/sql/create/model) statement to bring the OpenAI models to MindsDB.
...
FROM openai
USING
  api_key= 'YOUR_OPENAI_API_KEY' - `OPENAI_API_KEY` ENVIRONMENT VARIABLE VALUE
```
