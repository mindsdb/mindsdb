...
<Steps>
  <Step title="Create a knowledge base">
    ...
    my_kb = project.knowledge_bases.create(
        ...
        storage=server.databases.mydb.tables.vector_store,
  ...
  <Step title="Get the most relevant search results">
    ...
    ```python
    results = kb.find('color').filter(relevance>=0.2502).fetch()
    ```
  ...
  <Step title="Filter results by metadata">
    ...
    ```sql
    results = kb.find('color').filetr(product='Wireless Mouse').filter(relevance>=0.2502).fetch()
    ```
...
TODO: os this available in python sdk?
...
TODO
```python
results = my_kb.find('color').fetch()
```
...
TODO
```python
results = my_kb.find('color', limit=20).filter(relevance>=0.5).fetch()
```
...
TODO
```python
results = my_kb.find('color').filter(relevance>=0.5).fetch()
```
...
TODO
```python
results = my_kb.find('color', limit=2).fetch()
```
...
TODO
```python
results = my_kb.find().filter(product='Wireless Mouse').fetch()
```
...
TODO
```python
results = my_kb.find('color').filter(product='Wireless Mouse').filter(relevance>=0.5).fetch()
```
