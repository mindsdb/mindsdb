...
<Note>
When using [MindsDB via Docker](/setup/self-hosted/docker-desktop), the `storage` parameter is optional. The default storage is PGVector that is integrated into the MindsDB's Docker image.

When using [MindsDB via PyPI](/contribute/install), the `storage` parameter is required. Users need to connect their vector storage, such as PGVector, using the `CREATE DATABASE` command and then use it for the storage of the knowledge bases.
</Note>
