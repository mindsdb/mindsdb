...
Additional setup for the HTTP API on Linux and MacOS:

* `restart_on_failure`: If it is set to `True` (and `max_restart_count` is not reached), the restart of MindsDB will be attempted after the MindsDB process was killed with code 9.
* `max_restart_count`: This defines how many times the restart attempts can be made. Note that 0 stands for no limit.
* `max_restart_interval_seconds`: This defines the interval between consecutive restart attempts.
