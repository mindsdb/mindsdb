...
Users can edit the config values through HTTP requests or MindsDB Editor <todo: how to do that>. In this case, the `config.auto.json` file is created to store the updated `config.json` file. The `config.auto.json` file is stored in the location defined in the `paths.root` config parameter.
