...
The `default_llm` parameter specifies the default LLM that will be used with [knowledge bases](/mindsdb_sql/knowledge-bases), the [`LLM()` function](/mindsdb_sql/functions/llm_function), the [`TO_MARKDOWN()` function](/mindsdb_sql/functions/to_markdown_function), and [agents](/mindsdb_sql/agents/agent).
...
The `default_embedding_model` parameter specifies the default embedding model used with knowledge bases.
...
Learn more about the parameters following the [documentation of the `embedding_model` of knowledge bases](/mindsdb_sql/knowledge-bases#embedding-model).
</Accordion>
