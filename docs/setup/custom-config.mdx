...
<Accordion title="Config parameters">
...
Users can modify config values by directly editing the `config.json` file.

Users can edit the config values through an HTTP request or MindsDB Editor. In this case, the `config.auto.json` file is created to store the updated `config.json` file. The `config.auto.json` file is stored in the location defined in the `paths.root` config parameter.

<Tip>
Modifying config values through MindsDB Editor, provided that `"http_auth_enabled": true`, can be done in Settings.

Modifying config values through an HTTP request:

```bash
curl --request PUT \
  --url http://127.0.0.1:47334/api/config \
