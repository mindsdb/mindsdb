...
-   `-p 47335:47335` publishes port 47335 to access MindsDB MySQL API. This is optional and can be omitted if you don't need to use the MySQL API.
...
```bash
docker run --name mindsdb_container -e MKL_SERVICE_FORCE_INTEL=1 -p 47334:47334 mindsdb/mindsdb
```
</Warning>
...
```bash
docker run --name mindsdb_container -e MINDSDB_USERNAME='admin' -e MINDSDB_PASSWORD='password' -p 47334:47334 mindsdb/mindsdb
```
</Tip>
