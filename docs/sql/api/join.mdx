...
```sql
SELECT d.column_name, m1.column_name, m2.column_name, ...
FROM integration_name.table_name [AS] d
...
| `project_name.model_name_2`         | Optionally, you can join another model with the same input data in one query. |
