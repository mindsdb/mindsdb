...
You can easily create a chatbot to respond to tweets using jobs.
...
The [`SELECT`](/sql/api/select) statement joins the data table with the model table to get responses for tweets posted after the `{{PREVIOUS_START_DATETIME}}` timestamp. And then, the [`INSERT INTO`](/sql/api/insert) statement writes these responses to the `tweets` table of the `my_twitter` integration.
