...
### Querying Jobs [todo-testing&docs]
...
### Querying Jobs History [todo-testing&docs]
...
## Example 2 [todo-testing]

In this example, the job creates a table named as `result_{{START_DATE}}` and inserts predictions into it.

```sql
CREATE JOB slice_predictions (
   INSERT INTO int1.`result_{{START_DATETIME}}`( -- todo - should be create table instead of insert into
      SELECT *
      FROM example_db.demo_data.home_rentals
...
<Tip>
Please note that the uniqueness of the created table name is ensured here by using the `{{START_DATE}}` variable.
