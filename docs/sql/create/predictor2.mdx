...
To check the predictor status, query the [`mindsdb.models`](/sql/table-structure/#the-predictors-table) table.
...
To learn more about all the `model` options, visit the [Lightwood documentation page](https://lightwood.io/mixer.html).
...
## `CREATE MODEL` For Time Series Models
...
<Warning>
    **Getting a Prediction from a Time Series Model** Due to the nature of time series forecasting, you need to use the [`JOIN`](/sql/api/join) statement to get results.
</Warning>
