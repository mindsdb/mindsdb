...
sidebarTitle: Customer reviews notifcations
---
...
```sql
SELECT * FROM mindsdb.jobs WHERE name = 'customer_reviews_and_reponses_notifications';

SELECT * FROM log.jobs_history WHERE project = 'mindsdb' AND name = 'customer_reviews_and_reponses_notifications';
```
