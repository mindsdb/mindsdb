...
Notice that the `USING` clause is optional, but you must pass an API key eventually (either at model creation, engine creation, model usage, or in the mindsdb configuration file). The only exception to this is when using `cloud.mindsdb.com`. If you want your own token please register [here](https://www.nixtla.io/) (Submit Interest).
