...
*P.S.: You need to first create a *Mongo database* (here `date_fruit_db`) and add the `date_fruit_data` csv file. Then later you connect the above database with this one.*
...
On successful query run, you can see the `predictor` model under `mindsdb` database. Also, you have to wait for a few minutes for the model to get trained.
