...
## Data Setup
If you're going to use **MindsDB Cloud's files database** for connecting to the MongoDB database, then follow the steps as shown below--
...
3. Import the `.csv` file (here it is `Date_fruit.csv`), name the data source table that is used to store the file (here it is `date_fruit_data`), and click on the `Save and Continue` button.
