...
In this tutorial, you will learn how to use MindsDB and Hugging Face to perform **translation** tasks on a multilingual dataset.
...
Nice, you have 11146 rows/records in your new database and you're on our way!

## Connecting the Data

Now you know how to build and copy data to a table with PostgreSQL in your command line. 
Now it's time to connect with MindsDB:
...
- Once your status reads, `complete` it is time to start writing queries:
...
***Keep in mind that foreign sayings don't always translate to English exactly as we'd expect. For example, I was under the i,pression that the Hindi expression below meant, 'I love you'*** 

```sql
SELECT *
FROM hi_en
WHERE text_hi = 'mujhe tumase pyaar hai';
```

| PRED | text_hi |
| ---- | ------- |
| And you're going to have a choice. | mujhe tumase pyaar hai |

- Let's really challenge this predictor now and see how it responds when we input Urdu:
...
As can be seen in this tutorial, MindsDB's integration with Hugging Face gives you the power to perform Natural Language Processing tasks quickly and easily, using the database language, SQL. In just a few steps, you've already learned how to use MindsDB to translate multilingual data to English. As you probably already know, there are tons of real world scenarios that this newfound knowledge will be useful. Now that you've got your hands dirty, what are some more ways you can use MindsDB for NLP? Reach out to 
