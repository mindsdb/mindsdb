...
Now that you've got some data to work with, it's time to use MindDB and Hugging Face to perform some NLP operations.
...
- Start by creating a model to translate Spanish to English:
...
### Translating Hindi (Urdu) to English
...
```sql
CREATE MODEL translation_model_1
PREDICT translation
USING
   engine = 'huggingface',
   task = 'translation',
