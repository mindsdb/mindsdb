...
```bash
CREATE DATABASE mongodb_demo_db
WITH ENGINE = "mongodb",
PARAMETERS = {
  "host": "mongodb+srv://<username>:<password>@cluster0.0qmac.mongodb.net/",
  "database": "public"
};
```
...
```bash
SELECT *
FROM mongodb_demo_db.movie_reviews;
```
...
| _id                      | review                                                                                                                                                            |
| ------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 64129b66e24d840208b2e498 | it was good                                                                                                                                                       |
| 64129d44e24d840208b2e49a | this is a film well worth seeing , talking and singing heads and all .                                                                                            |
| 64129d64e24d840208b2e49b | ultimately , it ponders the reasons we need stories so much .                                                                                                     |
| 64129d76e24d840208b2e49c | everytime you think undercover brother has run out of steam , it finds a new way to surprise and amuse                                                            |
| 64129d8be24d840208b2e49d | it's a feel-bad ending for a depressing story that throws a bunch of hot-button items in the viewer's face and asks to be seen as hip, winking social commentary. |
```
...
```bash
CREATE MODEL sentiment_classifier_openai
PREDICT sentiment
USING
engine = 'openai',
...
```bash
SELECT * FROM models
WHERE name = 'sentiment_classifier_openai';
```
...
FROM sentiment_classifier_openai
WHERE review = 'it was good';
```
...
FROM mongodb_demo_db.movie_reviews AS input
JOIN sentiment_classifier_openai AS output
LIMIT 5;
```
...
| ultimately , it ponders the reasons we need stories so much .                                                                                                     | neutral   |
| everytime you think undercover brother has run out of steam , it finds a new way to surprise and amuse                                                            | positive  |
| it's a feel-bad ending for a depressing story that throws a bunch of hot-button items in the viewer's face and asks to be seen as hip, winking social commentary. | negative  |
```
