...
Before you start here, be sure to specify which database you're working inside of:

`USE mindsdb;`

```sql
CREATE MODEL predictor_spam1
PREDICT SPAM
USING
engine = 'huggingface',
task = 'text-classification',
...
| ---- | ------ | ------- | ------- | ------ | -------- | ------- | ------------- | --------------- | ----- | ----------------- | ---------------- | ---------------------- | --------------------- | ------------------- | --- | ---------- |
| predictor_spam1 | huggingface | mindsdb | 1 | complete | [NULL] | SPAM | up_to_date | 23.3.4.0 | [NULL] | [NULL] | {'target': 'SPAM', 'using': {'task': 'text-classification', 'model_name': 'mariagrandury/roberta-base-finetuned-sms-spam-detection', 'input_column': 'text_input', 'labels': ['spam', 'ham']}} | [NULL] | [NULL] | [NULL] | [NULL] | 2023-03-24 13:29:12.709293 |
```

Make sure this output reads `complete` before moving forward.

### Training a Predictor

Now that we've given our model data to learn from and it's training status is complete, let's go ahead and start making some predictions. 

I'm going to plug in some keywords that most of us probably associate with spam and see what our new prediction model thinks:

**input:**
```sql
SELECT *
FROM mindsdb.predictor_spam1
WHERE text = "Free";
```

**output:**
```sql
| spam | spam_explain | text |
| ---- | ------------ | ---- |
| spam | {"ham":0.0039043687283992767,"spam":0.996095597743988} | Free |
```
...
| spam | spam_explain | text |
| ---- | ------------ | ---- |
| spam | {"ham":0.008968036621809006,"spam":0.9910320043563843} | Fast cash! |
```

Now I'm going to input a few spam-looking messages and see what happens:

```sql
SELECT *
FROM mindsdb.predictor_spam1
WHERE text = "Free entry in 2 a wkly comp to win FA Cup final tkts 21st May 2005. Text FA to 87121 to receive entry question(std txt rate)T&C's apply 08452810075over18's ";
```

```output
| SPAM | SPAM_explain | text_input |
| ---- | ------------ | ---------- |
| ham | {"ham":0.9993841648101807,"spam":0.0006158568430691957} | NFree entry in 2 a wkly comp to win FA Cup final tkts 21st May 2005. Text FA to 87121 to receive entry question(std txt rate)T&C's apply 08452810075over18's |
```

Interestingly, our predictor identified this message has "ham". This simply tells us it needs some more training.