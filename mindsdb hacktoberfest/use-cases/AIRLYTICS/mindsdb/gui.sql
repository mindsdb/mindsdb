-- Connect MindsDB to your Google Sheet containing airline reviews
...
-- Create the core Knowledge Base for semantic airline review search and analytics
...
USING
  ...
  -- reranking_model is optional. Uncomment if needed.
  -- reranking_model = {
  --   "provider": "openai",
  --   "model_name": "gpt-4o",
  ...
  metadata_columns = [
    ...
    'type_of_traveller', 'seat_type', 'seat_comfort',
...
-- Insert your airline data into the Knowledge Base
...
-- Create an AI Agent to interpret analytical and semantic user queries
...
-- Test the Analytics Query Agent
SELECT answer
FROM analytics_query_agent
WHERE
  question = 'Among users complaining about flight delays, what % of those rated ground service above 4?';

--------------------------------------------------------------------

-- Create an AI Agent for Insight Interpretation and Decision Support
...
-- Test the Insight Interpreter Agent
SELECT answer
FROM insight_interpreter_agent
WHERE question = 'What can we infer from low Wi-Fi and ground service ratings?';

--------------------------------------------------------------------

-- Set up a data ingestion JOB to periodically check for new reviews and update the KB
CREATE JOB airline_kb_job AS (
  ...
  SELECT unique_id, review, airline_name, overall_rating, verified, aircraft,
         ...
         wifi_connectivity, value_for_money, recommended
  FROM airline_sheet_10000.airline_review_10000
  WHERE unique_id > 10000 -- Adjust if your dataset grows
)
EVERY 1 minute;

--------------------------------------------------------------------

-- Monitor or manage the job
...
-- Optional: Drop databases or KBs if you need to reset
DROP KNOWLEDGE_BASE airline_kb_10000;
DROP DATABASE airline_sheet_10000;
