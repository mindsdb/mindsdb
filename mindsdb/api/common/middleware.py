...
def verify_token(token: str) -> None:
    success = SIMPLE_USER_TOKEN is not None and token == SIMPLE_USER_TOKEN
    logger.debug(f"Given token: {token}. Expected token: {SIMPLE_USER_TOKEN}")
    logger.debug(f"Verify token result: {success}")
    return success
