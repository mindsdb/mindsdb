...
class ListIntegration(Resource):
    def get(self):
        return {
            'integrations': [k for k in request.datasource_interface.get_all(sensitive_info=False)]
        }
...
class AllIntegration(Resource):
    ...
    def get(self):
        integrations = request.datasource_interface.get_all(sensitive_info=False)
        return integrations
...
class Integration(Resource):
    ...
    def get(self, name):
        integration = request.datasource_interface.get(name, sensitive_info=False)
    ...
    def put(self, name):
        ...
        try:
            if 'enabled' in params:
                ...
                del params['enabled']
            request.datasource_interface.add(name, params)
    ...
    def delete(self, name):
        integration = request.datasource_interface.get(name)
        if integration is None:
            abort(400, f"Nothing to delete. '{name}' not exists.")
        try:
            request.datasource_interface.delete(name)
        except Exception as e:
            log.error(str(e))
    ...
    def post(self, name):
        ...
        try:
            if 'enabled' in params:
                params['publish'] = params['enabled']
                del params['enabled']
            request.datasource_interface.modify(name, params)
            DatabaseWrapper(request.company_id).setup_integration(name)
        except Exception as e:
            log.error(str(e))
...
class Check(Resource):
    ...
    def get(self, name):
        ...
        if request.datasource_interface.get(name) is None:
            abort(404, f'Can\'t find database integration: {name}')
