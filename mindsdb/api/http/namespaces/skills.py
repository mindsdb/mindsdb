...
def create_skill(project_name, skill):
    ...
    for required_field in ["name", "type", "params"]:
        if required_field not in skill:
            return http_error(
                HTTPStatus.BAD_REQUEST, "Missing field", 'Missing "{}" field for skill'.format(required_field)
            )
    name = skill["name"]
    type = skill["type"]
    params = skill["params"]
...
class SkillsResource(Resource):
    ...
    def post(self, project_name):
        ...
        if "skill" not in request.json:
            return http_error(
                HTTPStatus.BAD_REQUEST, "Missing parameter", 'Must provide "skill" parameter in POST body'
            )
...
class SkillResource(Resource):
    ...
    def get(self, project_name, skill_name):
        ...
        if existing_skill is None:
            return http_error(HTTPStatus.NOT_FOUND, "Skill not found", f"Skill with name {skill_name} not found.")
    ...
    def put(self, project_name, skill_name):
        ...
        updated_skill = skills_controller.update_skill(
            skill_name, new_name=new_name, project_name=project_name, type=new_type, params=new_params
        )
