...
class OkPacket(Packet):
    ...
    def setup(self):
        ...
        if state_track is not None:
            ...
            self.server_status = Datum("int<2>", status)
            self.state_track = b""
            self.state_track += Datum(
                "string<lenenc>", ""
            ).toStringPacket()  # 'info' - human readable status information
            for el in state_track:
                ...
                accum += (
                    struct.pack("i", SESSION_TRACK.SESSION_TRACK_SYSTEM_VARIABLES)[:1]
                    + struct.pack("i", len(part))[:1]
                    + part
                )
                # self.state_track
