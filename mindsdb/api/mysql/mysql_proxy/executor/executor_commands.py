...
class ExecuteCommands:
    ...
    def answer_describe_predictor(self, predictor_value):
        ...
        if predictor_name not in [x["name"] for x in models]:
            raise ErBadTableError(
                f"Can't describe predictor. There is no predictor with name '{predictor_name}'"
            )
        ...
        if predictor_attr is None:
            ...
            if predictor_attr == "features":
                ...
                columns = [Column(name="ensemble", table_name="", type="str")]
            else:
                raise ErNotSupportedYet(
                    "DESCRIBE '%s' predictor attribute is not supported yet"
                    % predictor_attr
                )
        ...
        if is_cloud and ctx.user_class == 0:
            ...
            for p in models:
                if (
                    ...
                    and p.training_start_at is not None
                    and p.training_stop_at is None
                ):
            ...
            if longest_training is not None and longest_training > datetime.timedelta(
                hours=1
            ):
                raise SqlApiException(
