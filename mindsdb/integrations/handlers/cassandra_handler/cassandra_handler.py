...
class CassandraHandler(DatabaseHandler):
    """
    ...
    def connect(self):
        """
        Handles the connection to a Cassandra/Scylla cluster.
        """
    ...
    def check_connection(self) -> StatusResponse:
        ...
        try:
            ...
            logger.error(f'Error connecting to Cassandra {self.connection_args["keyspace"]}, {e}!')
    ...
    def get_tables(self) -> Response:
        ...
        # Rename column from 'name' (system table output) to 'table_name' if needed
        # Note: Scylla handler implementation had: result.data_frame = df.rename(columns={df.columns[0]: 'table_name'})
        # Cassandra system tables vs Scylla might differ slightly, but DESCRIBE TABLES usually returns just names.
        # Check previous CassandraHandler implementation: df = df.rename(columns={'name': 'table_name'})
        # I will stick to a robust rename of the first column or check column names.
        # Let's use the logic I saw in CassandraHandler before:
        if 'name' in df.columns:
            df = df.rename(columns={'name': 'table_name'})
        elif 'keyspace_name' in df.columns and 'table_name' in df.columns:
             pass # Already good
        else:
             # Fallback to renaming first column
             if len(df.columns) > 0:
