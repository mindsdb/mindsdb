...
class EventStoreDB(DatabaseHandler):
    ...
    headers = {"Accept": "application/json", "ES-ResolveLinkTo": "True"}
    tlsverify = False
    ...
    def __init__(self, name, **kwargs):
        ...
        if (
            connection_data.get("tls") is not None
            and isinstance(connection_data.get("tls"), bool)
    ...
    def query(self, query: ASTNode) -> Response:
        if type(query) == Select:
                            ...
                            if "relation" in link:
                                if link["relation"] == "next":
                                    ...
                                    response = requests.get(
                                        build_next_url(link["uri"], self.read_batch_size),
                                        params=params,
            ...
            return Response(RESPONSE_TYPE.ERROR, error_message="Only 'select' queries are supported for EventStoreDB")
    ...
    def get_tables(self) -> Response:
        ...
        df = pd.DataFrame(streams, columns=["table_name"])
        return Response(RESPONSE_TYPE.TABLE, df)
    ...
    def get_columns(self, table_name) -> Response:
        ...
        if entry is None:
            return Response(RESPONSE_TYPE.ERROR, "Could not retrieve JSON event data to infer column types.")
        data = []
