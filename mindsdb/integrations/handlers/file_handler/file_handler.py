...
from mindsdb_sql.parser.ast import DropTables, Select, Star, Identifier
...
class FileHandler(DatabaseHandler):
    ...
    def query(self, query: ASTNode) -> Response:
        if type(query) is DropTables:
            ...
            query.targets = [
                Star() if isinstance(target, Identifier) and isinstance(target.parts[-1], Star) else target for target in query.targets
            ]
