...
class FrappeClient(object):
    ...
    def get_document(self, doctype: str, name: str) -> Dict:
        ...
        """
        document_response = requests.get(f"{self.base_url}/resource/{doctype}/{name}", headers=self.headers)
        if not document_response.ok:
            document_response.raise_for_status()
        return document_response.json()["data"]

    def get_documents(
        self, doctype: str, limit: int = None, fields: List[str] = None, filters: List[List] = None
    ) -> List[Dict]:
        ...
        documents_response = requests.get(
            f"{self.base_url}/resource/{doctype}/", params=params, headers=self.headers, allow_redirects=False
        )
        if documents_response.is_redirect:
            ...
            documents_response = requests.get(redirect_url, params=params, headers=self.headers, allow_redirects=False)

        if not documents_response.ok:
            documents_response.raise_for_status()
        return documents_response.json()["data"]

    def post_document(self, doctype: str, data: Dict):
        ...
        """
        post_response = requests.post(f"{self.base_url}/resource/{doctype}", json=data, headers=self.headers)
    ...
    def ping(self) -> bool:
        ...
        user_response = requests.get(f"{self.base_url}/method/frappe.auth.get_logged_user", headers=self.headers)
