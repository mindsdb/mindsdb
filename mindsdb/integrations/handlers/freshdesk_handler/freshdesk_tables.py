...
class FreshdeskAgentsTable(APITable):
    ...
    def select(self, query: ast.Select) -> pd.DataFrame:
        ...
        subset_where_conditions, filter_conditions = self.get_conditions(
            where_conditions)
        ...
        for op, arg1, arg2 in where_conditions:
            if arg1 in self.get_columns():
                if arg1 in self.get_api_filter_columns() and op == '=':
                    filter_conditions[self.get_api_filter_columns()[
                        arg1]] = arg2
                else:
    ...
    def get_freshdesk_agents(self, api_filters):
        agents = self.handler.freshdesk_client.agents.list_agents(
            **api_filters)
        response = []
...
class FreshdeskTicketsTable(APITable):
    ...
    def get_freshdesk_tickets(self, filter_conditions):
        if len(filter_conditions) > 0:
            ...
            tickets = self.handler.freshdesk_client.tickets.list_tickets(
                filter_name=None)
