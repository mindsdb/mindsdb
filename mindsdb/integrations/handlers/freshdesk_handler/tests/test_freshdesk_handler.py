...
class TestFreshdeskHandler(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.kwargs = {"connection_data": {"domain": "test.freshdesk.com", "api_key": "test_api_key_123"}}
    ...
    def test_agents_table_get_conditions(self):
        ...
        subset_conditions, filter_conditions = self.agents_table.get_conditions(where_conditions)

        # Check that API filter conditions are properly extracted
        expected_filter_conditions = {"email": "test@example.com", "mobile": "+1234567890"}
    ...
    def test_tickets_table_get_conditions(self):
        ...
        where_conditions = [["=", "status", "open"], ["=", "priority", "high"], [">", "id", 100], ["=", "group_id", 5]]

        subset_conditions, search_conditions = self.tickets_table.get_conditions(where_conditions)

        # Check that API filter conditions are properly extracted
        expected_search_conditions = [("=", "status", 2), ("=", "priority", 3), ("=", "group_id", 5)]
    ...
    def test_tickets_table_priority_status_mapping(self):
        ...
        where_conditions = [["=", "priority", "urgent"], ["=", "status", "closed"]]

        subset_conditions, search_conditions = self.tickets_table.get_conditions(where_conditions)

        # Check that string values are mapped to numbers
        expected_search_conditions = [("=", "priority", 4), ("=", "status", 5)]
        self.assertEqual(search_conditions, expected_search_conditions)

    def test_tickets_table_build_freshdesk_api_filter_query(self):
        """Test build_freshdesk_api_filter_query method."""
        conditions = [("=", "status", 2), ("=", "priority", 3)]

        result = self.tickets_table.build_freshdesk_api_filter_query(conditions)
