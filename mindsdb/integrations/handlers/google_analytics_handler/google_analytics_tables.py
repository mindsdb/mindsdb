...
class ConversionEventsTable(APITable):
    ...
    def insert(self, query: ast.Insert):
        ...
        if not set(columns).issubset(supported_columns):
            ...
            raise ValueError("Unsupported columns for create conversion event: " + ", ".join(unsupported_columns))
        params = {}
    ...
    def get_conversion_events(self, params: dict = None):
        ...
        while True:
            request = ListConversionEventsRequest(parent=url, page_token=page_token, **params)
    ...
    def create_conversion_event(self, params: dict = None):
        ...
        conversion_event = ConversionEvent(event_name=params["event_name"], counting_method=params["countingMethod"])
        request = CreateConversionEventRequest(conversion_event=conversion_event, parent=url)
    ...
    def update_conversion_event(self, params: dict = None):
        ...
        conversion_event = ConversionEvent(name=params["name"], counting_method=params["countingMethod"])
    ...
    def concat_dataframes(self, existing_df, data):
        ...
        """
        return pd.concat([existing_df, pd.DataFrame(data, columns=self.get_columns())], ignore_index=True)
