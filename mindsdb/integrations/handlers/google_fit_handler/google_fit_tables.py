...
class GoogleFitTable(APITable):
    def time_parser(self, args) -> int:
        ...
        time = pytz.timezone(str(get_localzone())).localize(
            datetime.datetime(int(ymd[0].rstrip()), int(ymd[1].rstrip()), int(ymd[2].rstrip()))
        )
    ...
    def select(self, query: ast.Select) -> Response:
        ...
        if not params:
            ...
            params["end_time"] = now
        result = self.handler.call_google_fit_api(method_name="get_steps", params=params)
        return result
