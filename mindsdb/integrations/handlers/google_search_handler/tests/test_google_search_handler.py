...
class GoogleSearchConsoleHandlerTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        cls.kwargs = {
            "connection_data": {"credentials": "/path/to/credentials.json"},
    ...
    def test_2_select_analytics_query(self):
        query = (
            "SELECT clicks FROM my_console.Analytics WHERE siteUrl = "
    ...
    def test_3_select_sitemaps_query(self):
        query = (
            "SELECT type FROM my_console.Sitemaps WHERE siteUrl = "
    ...
    def test_4_insert_sitemaps_query(self):
        query = "INSERT INTO my_console.Sitemaps VALUES ('https://www.mindsdb.com/sitemap.xml','https://www.mindsdb')"
        result = self.handler.native_query(query)
        assert result.type is RESPONSE_TYPE.TABLE

    def test_5_delete_sitemaps_query(self):
        query = (
            "DELETE FROM my_console.Sitemaps WHERE "
