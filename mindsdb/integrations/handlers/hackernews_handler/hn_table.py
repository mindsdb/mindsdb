...
class HNStoriesTable(APITable):
    def select(self, query: ast.Select) -> pd.DataFrame:
        ...
        # Call the Hacker News API to get the top stories
        url = f'{hn_handler.base_url}/askstories.json'
        response = requests.get(url)
        data = response.json()
...
class JobStoriesTable(APITable):
    def select(self, query: ast.Select) -> pd.DataFrame:
        """Select data from the stories table and return it as a pandas DataFrame.
        Args:
            query (ast.Select): The SQL query to be executed.
        ...
        # Call the Hacker News API to get the top stories
        url = f'{hn_handler.base_url}/jobstories.json'
        response = requests.get(url)
        data = response.json()
    ...
    def get_columns(self):
        ...
        Returns:
            list: A list of column names for the stories table.
        """
        return ['id', 'time', 'title', 'url', 'score', 'type']
