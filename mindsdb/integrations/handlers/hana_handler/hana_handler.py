    ...
    def native_query(self, query: Text) -> Response:
        ...
        with connection.cursor() as cur:
            try:
                ...
                logger.error(f"Error running query: {query} on {self.connection_data['address']}!")
                response = Response(RESPONSE_TYPE.ERROR, error_code=0, error_message=str(programming_error))
                connection.rollback()
            except Exception as unknown_error:
                logger.error(f"Unknown error running query: {query} on {self.connection_data['address']}!")
