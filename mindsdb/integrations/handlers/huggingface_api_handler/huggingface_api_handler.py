...
class HuggingFaceInferenceAPIHandler(BaseMLEngine):
    ...
    def create_validation(target, args=None, **kwargs):
        ...
        if args["task"] not in get_in_df_supported_tasks():
            raise UnsupportedTaskException(
                f"The task {args['task']} is not supported by the Hugging Face Inference API engine."
            )
    ...
    def predict(self, df: Optional[pd.DataFrame] = None, args: Optional[Dict] = None) -> None:
        ...
        if args["task"] == "text-classification":
            ...
            nlp = NLP(api_key, endpoint)
            result_df = nlp.fill_mask_in_df(df, input_column, options, model_name)

        elif args["task"] == "summarization":
            nlp = NLP(api_key, endpoint)
            result_df = nlp.summarization_in_df(df, input_column, parameters, options, model_name)

        elif args["task"] == "text-generation":
            nlp = NLP(api_key, endpoint)
            result_df = nlp.text_generation_in_df(df, input_column, parameters, options, model_name)

        elif args["task"] == "question-answering":
            nlp = NLP(api_key, endpoint)
            result_df = nlp.question_answering_in_df(df, input_column, args["context_column"], model_name)

        elif args["task"] == "sentence-similarity":
            nlp = NLP(api_key, endpoint)
            result_df = nlp.sentence_similarity_in_df(df, input_column, args["input_column2"], options, model_name)

        elif args["task"] == "zero-shot-classification":
            nlp = NLP(api_key, endpoint)
            result_df = nlp.zero_shot_classification_in_df(
                df, input_column, args["candidate_labels"], parameters, options, model_name
            )
            ...
            nlp = NLP(api_key, endpoint)
            result_df = nlp.translation_in_df(df, input_column, lang_in, lang_out, options, model_name)
            ...
            result_df = cp.image_classification_in_df(df, input_column, model_name)

        elif args["task"] == "object-detection":
            cp = ComputerVision(api_key, endpoint)
            result_df = cp.object_detection_in_df(df, input_column, model_name)

        elif args["task"] == "automatic-speech-recognition":
            ap = AudioProcessing(api_key, endpoint)
            result_df = ap.automatic_speech_recognition_in_df(df, input_column, model_name)

        elif args["task"] == "audio-classification":
            ap = AudioProcessing(api_key, endpoint)
            result_df = ap.audio_classification_in_df(df, input_column, model_name)
