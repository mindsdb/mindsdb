...
class InstatusHandlerTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.handler = IntercomHandler(
            name="mindsdb_intercon", connection_data={"access_token": os.environ.get("INTERCOM_ACCESS_TOKEN")}
        )
    ...
    def test_8_update_article(self):
        df = pd.DataFrame(
            self.handler.call_intercom_api(endpoint="/articles", params={"page": 1, "per_page": 1})["data"][0]
        )
        _id = df["id"][0]
