...
import numpy as np
import pandas as pd
from langchain.agents import Tool, create_sql_agent, initialize_agent, load_tools
from langchain.agents.agent_toolkits import SQLDatabaseToolkit
from langchain.chains.conversation.memory import ConversationSummaryBufferMemory
from langchain.chat_models import (  # GPT-4 fails to follow the output langchain requires, avoid using for now
    ChatOpenAI,
)
from langchain.llms import OpenAI
from langchain.prompts import PromptTemplate
from langchain.utilities import GoogleSerperAPIWrapper
from mindsdb_sql import Insert, parse_sql

from mindsdb.integrations.handlers.langchain_handler.mindsdb_database_agent import (
    MindsDBSQL,
)
from mindsdb.integrations.handlers.openai_handler.openai_handler import (
    CHAT_MODELS,
    OpenAIHandler,
)

_DEFAULT_MODEL = (
    "gpt-3.5-turbo"  # TODO: enable other LLM backends (AI21, Anthropic, etc.)
)
...
class LangChainHandler(OpenAIHandler):
    ...
    def _get_serper_api_key(self, args, strict=True):
        ...
        if strict:
            raise Exception(
                f"Missing API key serper_api_key. Either re-create this ML_ENGINE specifying the `serper_api_key` parameter,\
                 or re-create this model and pass the API key with `USING` syntax."
            )  # noqa

    def create(self, target, args=None, **kwargs):
        self.write_privileges = args.get("using", {}).get(
            "writer", self.write_privileges
        )
    ...
    def create_validation(target, args=None, **kwargs):
        if "using" not in args:
            raise Exception(
                "LangChain engine requires a USING clause! Refer to its documentation for more details."
            )
        else:
            args = args["using"]
    ...
    def predict(self, df, args=None):
        ...
        executor = args[
            "executor"
        ]  # used as tool in custom tool for the agent to have mindsdb-wide access
        ...
        if "prompt_template" not in args and "prompt_template" not in pred_args:
            raise Exception(
                f"This model expects a prompt template, please provide one."
            )
        ...
        agent_creation_method = modal_dispatch.get(
            args.get("modal_dispatch", "default"), "default_completion"
        )
    ...
    def default_completion(self, df, args=None, pred_args=None):
        ...
        model_name = pred_args.get(
            "model_name", args.get("model_name", self.default_model)
        )
        agent_name = pred_args.get(
        ...
        pred_args["tools"] = (
            args.get("tools")
            if "tools" not in pred_args
            else pred_args.get("tools", [])
        )
        ...
        memory = ConversationSummaryBufferMemory(
            llm=llm, max_token_limit=model_kwargs.get("max_tokens", None)
        )
        ...
        description = {
            ...
            ],  # packed as list to avoid additional rows
    ...
    def _setup_tools(self, model_kwargs, pred_args, executor):
        def _mdb_exec_call(query: str) -> str:
            ...
            try:
                ...
                data = (
                    f"mindsdb tool failed with error:\n{str(e)}"  # let the agent know
                )
            return data
        ...
        def _mdb_exec_metadata_call(query: str) -> str:
            try:
                ...
                assert (
                    1 <= len(parts) <= 2
                ), "query must be in the format: `integration` or `integration.table`"
                ...
                if len(parts) == 2:
                    ...
                    try:
                        table_name_col = (
                            "TABLE_NAME" if "TABLE_NAME" in df.columns else "table_name"
                        )
                        ...
                        types = (
                            handler.get_columns(table_name).data_frame["Type"].to_list()
                        )
                        data += f"List of columns and types:\n"
                        data += "\n".join(
                            [
                                f"\tColumn: `{field}`\tType: `{typ}`"
        ...
        toolkit = (
            pred_args["tools"]
            if pred_args["tools"] is not None
            else self.default_agent_tools
        )
        tools = load_tools(toolkit)
        if model_kwargs.get("serper_api_key", False):
            search = GoogleSerperAPIWrapper(
                serper_api_key=model_kwargs.pop("serper_api_key")
            )
            tools.append(
                Tool(
    ...
    def describe(self, attribute: Optional[str] = None) -> pd.DataFrame:
        ...
        if attribute == "info":
            if info is None:
                ...
                raise Exception(
                    "This model needs to be used before it can be described."
                )
    ...
    def sql_agent_completion(self, df, args=None, pred_args=None):
        ...
        db = MindsDBSQL(
            ...
            metadata=args["executor"].session.integration_controller,
        )
