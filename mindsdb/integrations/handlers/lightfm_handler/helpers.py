...
def get_item_user_idx(args: dict, n_users, n_items, item_ids=None, user_ids=None):
    ...
    if item_ids and user_ids:
        item_idx = np.array([int(args["item_id_to_idx_map"][item_id]) for item_id in item_ids])
        user_idx = np.array([int(args["user_id_to_idx_map"][user_id]) for user_id in user_ids])
...
def get_user_item_recommendations(n_users: int, n_items: int, args: dict, item_ids, user_ids, model: lightfm.LightFM):
    """
    ...
    user_item_recommendations_df["item_id"] = (
        user_item_recommendations_df["item_idx"].astype("str").map(args["item_idx_to_id_map"])
    )
    user_item_recommendations_df["user_id"] = (
        user_item_recommendations_df["user_idx"].astype("str").map(args["user_idx_to_id_map"])
    )
...
def get_item_item_recommendations(
    ...
    if item_ids:
        ...
        item_idx_to_id_map = {key: val for key, val in args["item_idx_to_id_map"].items() if val in item_ids}
    ...
    similar_items_df["item_id_two"] = similar_items_df["item_idx"].map(args["item_idx_to_id_map"])
