...
class PagesTable(APITable):
    ...
    def select(self, query: ast.Select) -> pd.DataFrame:
        ...
        for condition in where_conditions:
            if condition[1] == "title":
                if condition[0] != "=":
                    raise ValueError(
                        f"Unsupported operator '{condition[0]}' for column '{condition[1]}' in WHERE clause."
                    )
                title = condition[2]
        ...
        select_statement_executor = SELECTQueryExecutor(pages_df, selected_columns, [], order_by_conditions)
    ...
    def get_pages(self, title: str = None, page_id: int = None, limit: int = 20):
        ...
        if search_query:
            return [
                self.convert_page_to_dict(connection.page(result, auto_suggest=False))
                for result in connection.search(search_query, results=limit)
            ]
        else:
            return [
                self.convert_page_to_dict(connection.page(result, auto_suggest=False))
                for result in connection.random(pages=limit)
            ]
