...
from mindsdb.integrations.handlers.mindsdb_handler.settings import ConnectionConfig
...
def _map_type(mysql_type_text: str) -> MYSQL_DATA_TYPE:
    ...
    try:
        ...
        logger.warning(f"MindsDB handler: unknown type: {mysql_type_text}, use TEXT as fallback.")
    ...
    for col in description:
        ...
        if type_int == C_TYPES.MYSQL_TYPE_BLOB:
            ...
            if col[7] == 16:  # and col[8] == 45
                ...
                logger.debug(f"MindsDB handler: unknown type code {col[7]}, use TEXT as fallback.")
                mysql_types.append(MYSQL_DATA_TYPE.TEXT)
            # endregion
        else:
            logger.warning(f"MindsDB handler: unknown type id={type_int} in column {col[0]}, use TEXT as fallback.")
            mysql_types.append(MYSQL_DATA_TYPE.TEXT)
...
class MindsDBHandler(DatabaseHandler):
    """
    This handler handles connection and execution of the MindsDB statements via MySQL wire protocol.
    """
    ...
    def is_connected(self):
        """
        Checks if the handler is connected to the MindsDB database.
    ...
    def connect(self):
        """
        Establishes a connection to a MindsDB database.
        ...
        try:
            ...
            logger.error(f"Error connecting to MindsDB {self.database}, {e}!")
            raise

    def disconnect(self):
        """
        Closes the connection to the MindsDB database if it's currently open.
        """
    ...
    def check_connection(self) -> StatusResponse:
        """
        Checks the status of the connection to the MindsDB database.
        ...
        try:
            ...
            logger.error(f"Error connecting to MindsDB {self.connection_data['database']}, {e}!")
    ...
    def native_query(self, query: str) -> Response:
        """
        Executes a SQL query on the MindsDB database and returns the result.
    ...
    def get_tables(self) -> Response:
        """
        Get a list with all of the tables in MindsDB selected database, including knowledge bases
        """
        sql = """
            SELECT
            ...
            WHERE
                TABLE_TYPE IN ('BASE TABLE', 'VIEW', 'KNOWLEDGE BASE')
