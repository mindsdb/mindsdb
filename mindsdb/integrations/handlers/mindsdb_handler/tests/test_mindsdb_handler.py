...
from mindsdb.integrations.handlers.mindsdb_handler.mindsdb_handler import MindsDBHandler
from mindsdb.api.executor.data_types.response_type import RESPONSE_TYPE

HANDLER_KWARGS = {
    "connection_data": {
        "host": os.environ.get("MDB_TEST_MINDSDB_HOST", "127.0.0.1"),
        "port": os.environ.get("MDB_TEST_MINDSDB_PORT", "3306"),
        "user": os.environ.get("MDB_TEST_MINDSDB_USER", "root"),
        "password": os.environ.get("MDB_TEST_MINDSDB_PASS", "supersecret"),
        "database": os.environ.get("MDB_TEST_MINDSDB_DB", "mdb_db_handler_test"),
    }
}
...
def seed_db():
    ...
    with open("mindsdb/integrations/handlers/mindsdb_handler/tests/seed.sql", "r") as f:
...
def handler(request):
    seed_db()
    handler = MindsDBHandler("test_mindsdb_handler", **HANDLER_KWARGS)
    yield handler
...
class TestMindsDBHandlerTables:
...
class TestMindsDBHandlerColumns:
...
class TestMindsDBHandlerDisconnect:
