...
class NebulaGraphHandler(DatabaseHandler):
    ...
    def connect(self):
        ...
        config.min_size = 0
        config.max_size = self.connection_data.get('session_pool_size', 10)
        config.idle_time = 0
        ...
        graph_space = self.connection_data.get('graph_space')
        host = self.connection_data.get('host')
        port = self.connection_data.get('port')

        self.session_pool = SessionPool(user, password, graph_space, [(host, port)])
    ...
    def check_connection(self) -> StatusResponse:
        ...
        try:
            ...
            logger.error(f'Error connecting to NebulaGraph {self.host}:{self.port}@{self.graph_space}: {e}')
    ...
    def get_tables(self) -> Response:
        ...
        try:
            ...
            if tags.type == RESPONSE_TYPE.TABLE and edges.type == RESPONSE_TYPE.TABLE:
    ...
    def get_columns(self, table_name) -> Response:
        ...
        try:
            ...
            if tables.type == RESPONSE_TYPE.TABLE:
                ...
                table_type = tables.data_frame[tables.data_frame['Name'] == table_name]['table_type'].values[0]
            else:
                return Response(
                    RESPONSE_TYPE.ERROR,
