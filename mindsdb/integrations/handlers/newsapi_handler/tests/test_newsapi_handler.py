...
class NewsApiHandlerTest(unittest.TestCase):
    ...
    def test_4_select(self):
        # table = self.handler.get_table("article")
        with self.assertRaises(NewsAPIException):
            res = self.handler.native_query('SELECT * FROM article WHERE query="google" AND sources="google.com"')
    ...
    def test_6_select(self):
        # table = self.handler.get_table("article")
        res = self.handler.native_query('SELECT * FROM article WHERE query="google" AND publishedAt >= "2023-03-23" AND  publishedAt <= "2023-04-23"')
    ...
    # def test_8_select(self):
    #     # table = self.handler.get_table("article")
    #     res = self.handler.native_query('SELECT * FROM article WHERE query="google" LIMIT 200')
    #     assert res.type is RESPONSE_TYPE.TABLE
    ...
    def test_10_select(self):
        ...
        with self.assertRaises(NotImplementedError):
            res = self.handler.native_query('SELECT * FROM article WHERE query="google" ORDER BY query')
