    ...
    def create_validation(target, args=None, **kwargs):
        ...
        try:
            ...
            if e.args[0].startswith("Not found"):
                raise Exception(
                    f"Could not retrieve version {args['version']} of model {args['model_name']}. Verify values are correct and try again.",
                    e,
                )

            elif e.args[0].startswith("Incorrect authentication token"):
                raise Exception(
                    "Provided api_key is Incorrect. Get your api_key here: https://replicate.com/account/api-tokens"
                )
    ...
    def predict(self, df: pd.DataFrame, args: Optional[Dict] = None) -> pd.DataFrame:
        ...
        def get_data(conditions):
            ...
            if isinstance(output, types.GeneratorType) and args.get("model_type") == "LLM":
                ...
                output = output[
                    -1
                ]  # Returns generated image for controlNet models as it outputs filter and generated image
            return output
