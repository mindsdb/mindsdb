...
class OrdersTable(APITable):
    ...
    def insert(self, query: ast.Insert) -> None:
        ...
        insert_statement_parser = INSERTQueryParser(
            ...
            supported_columns=[
                ...
                "source_sl",
                "title_sl",
                "carrier_identifier_sl",
                "requested_fulfillment_service_id_sl",
                "is_removed_sl",
    ...
    def update(self, query: ast.Update) -> None:
        ...
        update_statement_executor = UPDATEQueryExecutor(orders_df, where_conditions)
    ...
    def create_orders(self, order_data: List[Dict[Text, Any]]) -> None:
        ...
        for order in order_data:
            ...
            order_data_trimmed = {key: val for key, val in order.items() if key not in all_columns}
...
class InventoryLevelTable(APITable):
    ...
    def select(self, query: ast.Select) -> pd.DataFrame:
        ...
        ValueError
            If the query contains an unsupported condition
        """
        select_statement_parser = SELECTQueryParser(query, "inventory_level", self.get_columns())
        selected_columns, where_conditions, order_by_conditions, result_limit = select_statement_parser.parse_query()
        ...
        select_statement_executor = SELECTQueryExecutor(
            inventory_df, selected_columns, subset_where_conditions, order_by_conditions
        )
...
class LocationTable(APITable):
    ...
    def select(self, query: ast.Select) -> pd.DataFrame:
        ...
        ValueError
            If the query contains an unsupported condition
        """
        select_statement_parser = SELECTQueryParser(query, "locations", self.get_columns())
        selected_columns, where_conditions, order_by_conditions, result_limit = select_statement_parser.parse_query()

        locations_df = pd.json_normalize(self.get_locations(limit=result_limit))

        select_statement_executor = SELECTQueryExecutor(
            locations_df, selected_columns, where_conditions, order_by_conditions
        )
...
class CustomerReviews(APITable):
    ...
    def select(self, query: ast.Select) -> pd.DataFrame:
        ...
        ValueError
            If the query contains an unsupported condition
        """
        select_statement_parser = SELECTQueryParser(query, "customer_reviews", self.get_columns())
        selected_columns, where_conditions, order_by_conditions, result_limit = select_statement_parser.parse_query()

        customer_reviews_df = pd.json_normalize(self.get_customer_reviews(limit=result_limit))

        select_statement_executor = SELECTQueryExecutor(
            customer_reviews_df, selected_columns, where_conditions, order_by_conditions
        )
    ...
    def get_customer_reviews(self, **kwargs) -> List[Dict]:
        if self.handler.yotpo_app_key is None or self.handler.yotpo_access_token is None:
            raise Exception(
                "You need to provide 'yotpo_app_key' and 'yotpo_access_token' to retrieve customer reviews."
            )
...
class CarrierServiceTable(APITable):
    ...
    def select(self, query: ast.Select) -> pd.DataFrame:
        ...
        ValueError
            If the query contains an unsupported condition
        """
        select_statement_parser = SELECTQueryParser(query, "carrier_service", self.get_columns())
        selected_columns, where_conditions, order_by_conditions, result_limit = select_statement_parser.parse_query()

        carrier_service_df = pd.json_normalize(self.get_carrier_service())

        select_statement_executor = SELECTQueryExecutor(
            carrier_service_df, selected_columns, where_conditions, order_by_conditions
        )
    ...
    def delete(self, query: ast.Delete) -> None:
        ...
        delete_query_executor = DELETEQueryExecutor(carrier_services_df, where_conditions)
    ...
    def update(self, query: ast.Update) -> None:
        ...
        values_to_update, where_conditions = update_statement_parser.parse_query()
        carrier_services_df = pd.json_normalize(self.get_carrier_service())
        update_query_executor = UPDATEQueryExecutor(carrier_services_df, where_conditions)
...
class ShippingZoneTable(APITable):
    ...
    def select(self, query: ast.Select) -> pd.DataFrame:
        ...
        ValueError
            If the query contains an unsupported condition
        """
        select_statement_parser = SELECTQueryParser(query, "shipping_zone", self.get_columns())
        selected_columns, where_conditions, order_by_conditions, result_limit = select_statement_parser.parse_query()

        shipping_zone_df = pd.json_normalize(
            self.get_shipping_zone(), record_path="countries", meta=["id", "name"], record_prefix="countries_"
        )

        select_statement_executor = SELECTQueryExecutor(
            shipping_zone_df, selected_columns, where_conditions, order_by_conditions, result_limit
        )
...
class SalesChannelTable(APITable):
    ...
    def select(self, query: ast.Select) -> pd.DataFrame:
        ...
        select_statement_parser = SELECTQueryParser(query, "sales_channel", self.get_columns())
        selected_columns, where_conditions, order_by_conditions, result_limit = select_statement_parser.parse_query()

        sales_channel_df = pd.json_normalize(self.get_sales_channel(limit=result_limit))

        select_statement_executor = SELECTQueryExecutor(
            sales_channel_df, selected_columns, where_conditions, order_by_conditions, result_limit
        )
