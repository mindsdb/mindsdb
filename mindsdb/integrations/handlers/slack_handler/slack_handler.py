...
from mindsdb_sql.parser.ast import ASTNode, Delete, Update
...
class SlackChannelsTable(APITable):
    ...
    def select(self, query: ast.Select) -> Response:
        ...
        for op, arg1, arg2 in conditions:
            ...
            elif arg1 == "remove_bot_messages":
                if op == "=":
                    params["remove_bot_messages"] = bool(arg2)
                else:
                    raise NotImplementedError(f"Unknown op: {op}")
            else:
        ...
        try:
            ...
            if params.get("remove_bot_messages"):
                conversation_history = [
                    message
                    for message in conversation_history.copy()
        ...
        result = pd.DataFrame(conversation_history, columns=columns)
        ...
        if "start_time" in params or "end_time" in params:
            filtered_df = pd.DataFrame(
                response_datetime_timestamp, columns=result.columns
            )
    ...
    def insert(self, query):
        ...
        for row in query.values:
            ...
            if "channel" not in params or "response" not in params:
                raise Exception(
                    "To insert data into Slack, you need to provide the 'channel' and 'response' parameters."
                )

            # post message to Slack channel
            try:
                response = self.client.chat_postMessage(
                    channel=params["channel"], text=params["response"]
                )
            except SlackApiError as e:
                raise Exception(
    ...
    def update(self, query: ast.Update):
        ...
        try:
            response = self.client.chat_update(
                channel=params["channel"], ts=params["ts"], text=params["text"]
            )
        except SlackApiError as e:
            raise Exception(
                f"Error updating message in Slack channel '{params['channel']}' with timestamp '{params['ts']}' and message '{params['message']}': {e.response['error']}"
            )
