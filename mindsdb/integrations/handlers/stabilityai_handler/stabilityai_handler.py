...
class StabilityAIHandler(BaseMLEngine):
    ...
    def create(self, target: str, df: Optional[pd.DataFrame] = None, args: Optional[Dict] = None) -> None:
        if "using" not in args:
            raise Exception(
                "Stability AI Inference engine requires a USING clause! Refer to its documentation for more details."
            )
    ...
    def _process_text_image(self, df, args):
        def generate_text_image(conds, client):
            ...
            return client.text_to_image(
                prompt=conds.get("text"), height=conds.get("height", 1024), width=conds.get("width", 1024)
            )
        ...
        for col in df.columns:
            if col not in supported_params:
                raise Exception(
                    f"Unknown column {col}. Currently supported parameters for text to image - {supported_params}"
                )
    ...
    def _process_image_image(self, df, args):
        def generate_image_image(conds, client):
            ...
            return client.image_to_image(
                image_url=conds.get("image_url"),
                prompt=conds.get("text"),
                height=conds.get("height", 1024),
                width=conds.get("width", 1024),
            )
        ...
        for col in df.columns:
            if col not in supported_params:
                raise Exception(
                    f"Unknown column {col}. Currently supported parameters for image to image - {supported_params}"
                )
    ...
    def _process_image_upscaling(self, df, args):
        def generate_image_upscaling(conds, client):
            conds = conds.to_dict()
            return client.image_upscaling(
                image_url=conds.get("image_url"),
                prompt=conds.get("text"),
                height=conds.get("height"),
        ...
        for col in df.columns:
            if col not in supported_params:
                raise Exception(
                    f"Unknown column {col}. Currently supported parameters for image scaling - {supported_params}"
                )
    ...
    def _process_image_masking(self, df, args):
        def generate_image_mask(conds, client):
            ...
            return client.image_to_image(
                ...
                height=conds.get("height"),
                width=conds.get("width"),
                mask_image_url=conds.get("mask_image_url"),
            )
        ...
        for col in df.columns:
            if col not in supported_params:
                raise Exception(
                    f"Unknown column {col}. Currently supported parameters for image masking - {supported_params}"
                )
