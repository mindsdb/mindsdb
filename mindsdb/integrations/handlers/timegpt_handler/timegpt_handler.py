...
class TimeGPTHandler(BaseMLEngine):
    ...
    def predict(self, df, args={}):
        ...
        if model_args["mode"] == "forecasting":
            ...
            results_df["anomaly"] = (results_df["y"] > results_df[f"TimeGPT-hi-{model_args['level'][0]}"]) | (
                results_df["y"] < results_df[f"TimeGPT-lo-{model_args['level'][0]}"]
            )
    ...
    def describe(self, attribute=None):
        ...
        if attribute == "model":
            ...
            df = pd.DataFrame({"order by": [model_args["order_by"]], "target": model_args["target"]})
