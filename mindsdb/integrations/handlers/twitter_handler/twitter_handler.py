...
class TweetsTable(APITable):
    def select(self, query: ast.Select) -> Response:
        ...
        result = self.handler.call_twitter_api(method_name="search_recent_tweets", params=params, filters=filters)
    ...
    def insert(self, query: ast.Insert):
        ...
        for p in insert_params:
            if p not in self.handler.connection_args:
                raise Exception(
                    f"To insert data into Twitter, you need to provide the following parameters when connecting it to MindsDB: {insert_params}"
                )  # noqa
...
class TwitterHandler(APIHandler):
    ...
    def create_connection(self, api_version=2):
        if api_version == 1:
            auth = tweepy.OAuthHandler(self.connection_args["consumer_key"], self.connection_args["consumer_secret"])
            auth.set_access_token(self.connection_args["access_token"], self.connection_args["access_token_secret"])
