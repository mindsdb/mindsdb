...
class UnifyHandler(BaseMLEngine):
    ...
    def predict(self, df: Optional[pd.DataFrame] = None, args: Optional[Dict] = None) -> None:
        ...
        if "model" not in args["using"]:
            raise Exception(
                "Unify requires an model parameter in the USING clause! Refer to its documentation for more details."
            )
        model = args["using"]["model"]

        if "provider" not in args["using"]:
            raise Exception(
                "Unify requires a provider parameter in the USING clause! Refer to its documentation for more details."
            )
        ...
        if self.endpoint not in available_endpoints:
            raise Exception(
                "The model, provider or their combination is not supported by Unify! The supported endpoints are: "
                + str(available_endpoints)
            )
