...
class ProductsTable(APITable):
    ...
    def insert(self, query: ast.Insert) -> None:
        ...
        insert_statement_parser = INSERTQueryParser(
            query,
            supported_columns=['name', 'slug', 'type', 'status', 'featured', 'catalog_visibility', 'description', 'short_description', 'sku', 'regular_price', 'sale_price', 'date_on_sale_from', 'date_on_sale_from_gmt', 'date_on_sale_to', 'date_on_sale_to_gmt',
                               'virtual', 'downloadable', 'downloads', 'download_limit', 'download_expiry', 'external_url', 'button_text', 'tax_status', 'tax_class', 'manage_stock', 'stock_quantity', 'stock_status', 'backorders', 'sold_individually', 'weight',
                               'dimensions', 'shipping_class', 'reviews_allowed', 'upsell_ids', 'cross_sell_ids', 'parent_id', 'purchase_note', 'categories', 'tags', 'images', 'attributes', 'default_attributes', 'grouped_products', 'menu_order', 'meta_data'],
...
class CustomersTable(APITable):
    ...
    def insert(self, query: ast.Insert) -> None:
        ...
        insert_statement_parser = INSERTQueryParser(
            query,
            supported_columns=['email', 'first_name', 'last_name', 'username', 'password', 'billing', 'shipping', 'meta_data'],
    ...
    def update_customers(self, customer_ids: List[int], values_to_update: Dict[Text, Any]) -> None:
        wcapi = self.handler.connect()
...
class OrdersTable(APITable):
    ...
    def insert(self, query: ast.Insert) -> None:
        ...
        insert_statement_parser = INSERTQueryParser(
            query,
            supported_columns=['parent_id', 'status', 'currency', 'customer_id', 'customer_note', 'billing', 'shipping', 'payment_method', 'payment_method_title', 'transaction_id', 'meta_data', 'line_items', 'shipping_lines', 'fee_lines', 'coupon_lines','set_paid'],
...
class TaxesTable(APITable):
    ...
    def select(self, query: ast.Select) -> pd.DataFrame:
        ...
        selected_columns, where_conditions, tax_by_conditions, result_limit = select_statement_parser.parse_query()
    ...
    def insert(self, query: ast.Insert) -> None:
        ...
        insert_statement_parser = INSERTQueryParser(
            query,
            supported_columns=['country', 'state', 'postcode', 'city', 'postcodes', 'cities', 'rate', 'name', 'priority', 'compound', 'shipping', 'order', 'class'],
            mandatory_columns=['name'],
            all_mandatory=False
        )
    ...
    def update_taxes(self, tax_ids: List[int], values_to_update: Dict[Text, Any]) -> None:
        wcapi = self.handler.connect()
...
class SettingsTable(APITable):
    ...
    def select(self, query: ast.Select) -> pd.DataFrame:
        ...
        selected_columns, where_conditions, setting_by_conditions, result_limit = select_statement_parser.parse_query()
...
class CouponsTable(APITable):
    ...
    def select(self, query: ast.Select) -> pd.DataFrame:
        ...
        selected_columns, where_conditions, coupon_by_conditions, result_limit = select_statement_parser.parse_query()
    ...
    def insert(self, query: ast.Insert) -> None:
        ...
        insert_statement_parser = INSERTQueryParser(
            query,
            supported_columns=['code', 'amount', 'discount_type', 'description', 'date_expires', 'date_expires', 'individual_use', 'product_ids', 'excluded_product_ids', 'usage_limit', 'usage_limit_per_user', 'limit_usage_to_x_items', 'free_shipping', 'product_categories', 'excluded_product_categories','exclude_sale_items', 'minimum_amount', 'maximum_amount', 'email_restrictions', 'meta_data'],
    ...
    def update_coupons(self, coupon_ids: List[int], values_to_update: Dict[Text, Any]) -> None:
        wcapi = self.handler.connect()
...
class ShippingZonesTable(APITable):
    ...
    def select(self, query: ast.Select) -> pd.DataFrame:
        ...
        selected_columns, where_conditions, shipping_zone_by_conditions, result_limit = select_statement_parser.parse_query()
    ...
    def update_shipping_zones(self, shipping_zone_ids: List[int], values_to_update: Dict[Text, Any]) -> None:
        wcapi = self.handler.connect()
    ...
    def create_shipping_zones(self, shipping_zone_data: List[Dict[Text, Any]]) -> None:
        ...
        for shipping_zone in shipping_zone_data:
            ...
            if 'id' not in created_shipping_zone.json():
                ...
                logger.info(f'Shipping_zone {created_shipping_zone.json()["id"]} created')
...
class RefundsTable(APITable):   
    ...
    def select(self, query: ast.Select) -> pd.DataFrame:
        ...
        selected_columns, where_conditions, refund_by_conditions, result_limit = select_statement_parser.parse_query()
        ...
        #     page_num = 1
        #     while(val != 0):
        #         response = self.get_refunds(order_id, page=page_num, per_page=result_limit)
        #         all_refunds = all_refunds + response
        #         page_num = page_num + 1
        #         val = len(response)
                
        refunds_df = pd.json_normalize(all_orders)

        select_statement_executor = SELECTQueryExecutor(
            refunds_df,
            selected_columns,
            where_conditions,
            refund_by_conditions
        )
    ...
    def insert(self, query: ast.Insert) -> None:
        ...
        insert_statement_parser = INSERTQueryParser(
            query,
            supported_columns=['amount', 'reason', 'meta_data', 'line_items'],
            mandatory_columns=['code'],
            all_mandatory=False
        )
    ...
    def delete(self, query: ast.Delete) -> None:
        ...
        refunds_df = pd.json_normalize(self.get_refunds())
        ...
        self.delete_refunds(refund_ids)

    def update(self, query: ast.Update) -> None:
        """Updates data from the Shopify "PUT /refunds" API endpoint.
        ...
        refunds_df = update_query_executor.execute_query()
        refund_ids = refunds_df['id'].tolist()
        
        self.update_refunds(refund_ids, values_to_update)

    def get_columns(self) -> List[Text]:
        orders = self.get_orders(per_page=1)
        order_id = orders[0]['id']
        return pd.json_normalize(self.get_refunds(order_id, per_page=1)).columns.tolist()
    ...
    def update_refunds(self, order_ids: List[int], refund_ids: List[int], values_to_update: Dict[Text, Any]) -> None:
        wcapi = self.handler.connect()
        
        number_id = len(refund_ids)
        for i in range(number_id):
            refund = wcapi.get(f'orders/{order_ids[i]}/refunds/{refund_ids[i]}')
            for key, value in values_to_update.items():
                setattr(refund, key, value)
            wcapi.put(f'orders/{order_ids[i]}/refunds/{refund_ids[i]}', refund)
            logger.info(f'Refund {refund_ids[i]} of order {order_ids[i]} updated')

    def delete_refunds(self, refund_ids: List[int]) -> None:
        wcapi = self.handler.connect()
        for refund_id in refund_ids:
            wcapi.delete(f'refunds/{refund_id}')
    ...
    def create_refunds(self, refund_data: List[Dict[Text, Any]]) -> None:
        ...
        for refund in refund_data:
            created_refund = wcapi.post('refunds', refund)
...
class PaymentGatewaysTable(APITable):
    ...
    def select(self, query: ast.Select) -> pd.DataFrame:
        ...
        selected_columns, where_conditions, payment_gateway_by_conditions, result_limit = select_statement_parser.parse_query()

        payment_gateways_df = pd.json_normalize(self.get_payment_gateways(per_page=result_limit))
...
class ReportSalesTable(APITable):
    ...
    def select(self, query: ast.Select) -> pd.DataFrame:
        ...
        selected_columns, where_conditions, report_sale_by_conditions, result_limit = select_statement_parser.parse_query()
...
class ReportTopSellersTable(APITable):
    ...
    def select(self, query: ast.Select) -> pd.DataFrame:
        ...
        selected_columns, where_conditions, report_top_seller_by_conditions, result_limit = select_statement_parser.parse_query()
    ...
    def get_columns(self) -> List[Text]:
        return pd.json_normalize(self.get_report_top_sellers(per_page=1)).columns.tolist()
...
class SettingOptionTable(APITable):
    ...
    def select(self, query: ast.Select) -> pd.DataFrame:
        ...
        selected_columns, where_conditions, setting_option_by_conditions, result_limit = select_statement_parser.parse_query()
