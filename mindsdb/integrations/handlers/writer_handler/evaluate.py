...
class WriterEvaluator:
    ...
    def calculate_retrieval_metrics(self, df: pd.DataFrame, context_embeddings, retrieved_context_embeddings, prefix="retrieval_"):
        """Calculate retrieval metrics"""
        for metric in self.retrieval_metrics:
            col_name = f"{prefix}{metric}"
            if metric == "cosine_similarity":
                df[col_name] = self.metric_map[metric](context_embeddings, retrieved_context_embeddings)
            elif metric == "accuracy":
                col_name = f"{prefix}match"
                df[col_name] = self.get_matches(gt_embeddings=context_embeddings, test_embeddings=retrieved_context_embeddings, threshold=self.args.retriever_match_threshold)
            else:
    ...
    def calculate_generation_metrics(self, df: pd.DataFrame, generated_answer_embeddings, reference_answer_embeddings, prefix="generator_"):
        """Calculate generation metrics"""
        for metric in self.generator_metrics:
            col_name = f"{prefix}{metric}"
            if metric == "cosine_similarity":
                df[col_name] = self.calculate_cosine_similarities(generated_answer_embeddings, reference_answer_embeddings)
            elif metric == "accuracy":
                col_name = f"{prefix}match"
                df[col_name] = self.get_matches(gt_embeddings=reference_answer_embeddings, test_embeddings=generated_answer_embeddings, threshold=self.args.generator_match_threshold)
            elif metric in ["rouge", "bleu", "meteor"]:
                df[col_name] = getattr(self, f"calculate_{metric}_scores")(df)
            else:
                raise ValueError(f"metric {metric} not supported")
    ...
    def calculate_rouge_scores(self, df: pd.DataFrame) -> pd.Series:
        """Calculate ROUGE scores for each row in the DataFrame"""
        return df.apply(lambda x: self.metric_map["rouge"](x["generated_answers"], x["reference_answers"]), axis=1)

    def calculate_bleu_scores(self, df: pd.DataFrame) -> pd.Series:
        """Calculate BLEU scores for each row in the DataFrame"""
        return df.apply(lambda x: self.metric_map["bleu"](x["tokenized_generated_answers"], x["tokenized_reference_answers"]), axis=1)

    def calculate_meteor_scores(self, df: pd.DataFrame) -> pd.Series:
        """Calculate METEOR scores for each row in the DataFrame"""
        return df.apply(lambda x: self.calculate_meteor(x["tokenized_generated_answers"], x["tokenized_reference_answers"]), axis=1)
    ...
    def evaluation_prompt(self, question: str, context: str):
        ...
        if self.args.summarize_context:
            return self.rag.summarize_context(question=question, combined_context=context)
    ...
    def get_evaluation_prompts(self, df: pd.DataFrame) -> List[str]:
        ...
        return [self.evaluation_prompt(question, context) for question, context in zip(df["question"], df["retrieved_context"])]
    ...
    @staticmethod
    def _calculate_cosine_similarity(gt_embeddings: List[float], test_embeddings: List[float]) -> float:
        """Calculate cosine similarity between a context and retrieved context embedding"""
        return 1 - distance.cosine(gt_embeddings, test_embeddings)

    def calculate_cosine_similarities(self, gt_embeddings: List[List[float]], test_embeddings: List[List[float]]):
        """Calculate cosine similarity for each ground truth and retrieved/generated pair for a given question"""
        return [self._calculate_cosine_similarity(context_embedding, retrieved_context_embedding) for context_embedding, retrieved_context_embedding in zip(gt_embeddings, test_embeddings)]

    @staticmethod
    def check_match(cosine_similarity: float, threshold: float = 0.7) -> int:
        return int(cosine_similarity >= threshold)

    def get_matches(self, gt_embeddings, test_embeddings, threshold: float = 0.7) -> List[int]:
        """Get matches for each ground truth and retrieved/generated pair for a given question"""
        cosine_similarities = self.calculate_cosine_similarities(gt_embeddings=gt_embeddings, test_embeddings=test_embeddings)
        matches = [self.check_match(cosine_similarity, threshold=threshold) for cosine_similarity in cosine_similarities]
        return matches
    ...
    def extract_rogue_scores(self, df: pd.DataFrame, rogue_scores_col: str = "rouge_scores"):
        ...
        for rouge_metric in rouge_metrics:
            for supported_metric in supported_metrics:
                ...
                df[col_name] = df.apply(lambda x: getattr(x[rogue_scores_col][rouge_metric], supported_metric), axis=1)
                self.store_mean_metric(col_name=col_name, mean_metric=df[col_name].mean())
    ...
    @staticmethod
    def calculate_bleu(generated_tokens: List[str], reference_tokens: List[str]) -> float:
        return sentence_bleu([reference_tokens], generated_tokens)

    @staticmethod
    def calculate_meteor(generated_tokens: List[str], reference_tokens: List[str]) -> float:
    ...
    def evaluate_retrieval(self):
        ...
        df["retrieved_context"] = df.apply(lambda x: self.extract_returned_text(self.query_vector_store(x["question"])), axis=1)

        # embed context and retrieved context
        context_embeddings = self.embed_texts(df["context"].tolist())
        retrieved_context_embeddings = self.embed_texts(df["retrieved_context"].tolist())

        df = self.calculate_retrieval_metrics(df, context_embeddings, retrieved_context_embeddings)
    ...
    def evaluate_generation(self, df: pd.DataFrame):
        ...
        df["tokenized_generated_answers"] = df.apply(lambda x: self._tokenize(x["generated_answers"]), axis=1)
        df["tokenized_reference_answers"] = df.apply(lambda x: self._tokenize(x["reference_answers"]), axis=1)
        ...
        df = self.calculate_generation_metrics(df, generated_answer_embeddings, reference_answer_embeddings)
