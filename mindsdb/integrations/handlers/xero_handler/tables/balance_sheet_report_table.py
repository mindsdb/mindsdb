...
class BalanceSheetReportTable(XeroReportTable):
    """Table for Xero Balance Sheet Report"""
    ...
    SUPPORTED_FILTERS = {
        ...
        "tracking_option_id_1": {"type": "direct", "param": "tracking_option_id1"},
    ...
    def get_columns(self) -> List[str]:
        """
        Return column names for the balance sheet report.
    ...
    def select(self, query: ast.Select) -> pd.DataFrame:
        ...
        # Parse query to get result_limit (reports don't paginate, so we use default limit)
        parser = SELECTQueryParser(
            query, "balance_sheet_report", columns=self.get_columns()
        )
        ...
        try:
            # Fetch balance sheet report (single API call - no pagination)
            response = api.get_report_balance_sheet(
