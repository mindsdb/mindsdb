...
class ExecutiveSummaryReportTable(XeroReportTable):
    """Table for Xero Executive Summary Report"""
    ...
    def get_columns(self) -> List[str]:
        """
        Return column names for the executive summary report.
    ...
    def select(self, query: ast.Select) -> pd.DataFrame:
        """
        Fetch executive summary report from Xero API
        ...
        parser = SELECTQueryParser(
            query, "executive_summary_report", columns=self.get_columns()
        )
        ...
        try:
            # Fetch executive summary report
            response = api.get_report_executive_summary(
            ...
            raise Exception(f"Failed to fetch executive summary report: {str(e)}")
