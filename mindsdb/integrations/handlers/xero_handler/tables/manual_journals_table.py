...
class ManualJournalsTable(XeroTable):
    ...
    def select(self, query: ast.Select) -> pd.DataFrame:
        ...
        parser = SELECTQueryParser(
            query, "manual_journals", columns=self.get_columns(), use_default_limit=False
        )
        ...
        try:
            while result_limit is None or records_fetched < result_limit:
                ...
                # Fetch manual journals with pagination parameters
                response = api.get_manual_journals(
                ...
                if not response.manual_journals:
                    break  # No more data
                ...
                if hasattr(response, 'pagination') and response.pagination:
                    ...
                    if len(response.manual_journals) < records_to_fetch:
                        break
