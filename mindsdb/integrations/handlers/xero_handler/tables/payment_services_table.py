...
class PaymentServicesTable(XeroTable):
    """Table for Xero Payment Services"""
    ...
    def select(self, query: ast.Select) -> pd.DataFrame:
        ...
        try:
            # Fetch payment services with optimized parameters
            payment_services = api.get_payment_services(xero_tenant_id=self.handler.tenant_id, **api_params)
            df = self._convert_response_to_dataframe(payment_services.payment_services or [])
            df.rename(columns=self.COLUMN_REMAP, inplace=True)
        except Exception as e:
            raise Exception(f"Failed to fetch payment services: {str(e)}")
        ...
        parser = SELECTQueryParser(
            query, "payment_services", columns=self.get_columns()
        )
