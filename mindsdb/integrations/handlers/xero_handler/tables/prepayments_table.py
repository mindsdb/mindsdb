...
class PrepaymentsTable(XeroTable):
    ...
    def select(self, query: ast.Select) -> pd.DataFrame:
        ...
        parser = SELECTQueryParser(
            query, "prepayments", columns=self.get_columns(), use_default_limit=False
        )
        ...
        try:
            while result_limit is None or records_fetched < result_limit:
                ...
                # Fetch prepayments with pagination parameters
                response = api.get_prepayments(
                ...
                if not response.prepayments:
                    break  # No more data

                all_data.extend(response.prepayments)
                records_fetched += len(response.prepayments)
                ...
                if hasattr(response, 'pagination') and response.pagination:
                    ...
                    if len(response.prepayments) < records_to_fetch:
                        break
