...
class TrialBalanceReportTable(XeroReportTable):
    """Table for Xero Trial Balance Report"""
    ...
    def get_columns(self) -> List[str]:
        """
        Return column names for the trial balance report.
    ...
    def select(self, query: ast.Select) -> pd.DataFrame:
        """
        Fetch trial balance report from Xero API
        ...
        parser = SELECTQueryParser(
            query, "trial_balance_report", columns=self.get_columns()
        )
        ...
        try:
            ...
            response = api.get_report_trial_balance(
