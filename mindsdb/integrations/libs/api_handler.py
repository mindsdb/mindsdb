...
from mindsdb.integrations.utilities.sql_utils import extract_comparison_conditions, filter_dataframe, FilterCondition, FilterOperator, SortColumn
...
class FuncParser:
    def from_string(self, query_string):
        ...
        params = {keyword.arg: self.process(keyword.value) for keyword in body.keywords}
        return fnc_name, params

    def process(self, node):
        # ... (rest of the process method remains unchanged)
...
class APIResource(APITable):
    def select(self, query: Select) -> pd.DataFrame:
        conditions = [FilterCondition(i[1], FilterOperator(i[0].upper()), i[2]) for i in extract_comparison_conditions(query.where)]
        limit = query.limit.value if query.limit else None
        sort = [SortColumn(an_order.field.parts[-1], an_order.direction.upper() != 'DESC') for an_order in query.order_by] if query.order_by else None
        targets = [col.parts[-1] for col in query.targets if isinstance(col, Identifier)]
        
        result = self.list(conditions=conditions, limit=limit, sort=sort, targets=targets)
        
        filters = [[cond.op.value, cond.column, cond.value] for cond in conditions if not cond.applied]
    ...
    def insert(self, query: Insert) -> None:
        ...
        for a_row in query.values:
            self.add(dict(zip(columns, a_row)))
...
class APIHandler(BaseHandler):
    ...
    def get_columns(self, table_name: str) -> Response:
        ...
        return Response(RESPONSE_TYPE.TABLE, pd.DataFrame({'Field': result, 'Type': 'str'}))

    def get_tables(self) -> Response:
        result = list(self._tables.keys())
        return Response(RESPONSE_TYPE.TABLE, pd.DataFrame({'table_name': result, 'table_type': 'BASE TABLE'}))
