...
class BaseMLEngineExec:
    ...
    def learn(
        ...
        predictor_record = db.Predictor(
            ...
            training_metadata={"hostname": socket.gethostname(), "reason": "retrain" if is_retrain else "learn"},
        )
    ...
    def predict(
        ...
        kwargs = {"name": model_name, "ml_handler_name": self.name, "project_name": project_name}
        if version is None:
            kwargs["active"] = True
        else:
            kwargs["active"] = None
        ...
        args = {"pred_format": pred_format, "predict_params": using, "using": using}
    ...
    def finetune(
        ...
        search_args = {"active": None, "name": model_name, "status": PREDICTOR_STATUS.COMPLETE}
        ...
        self.create_validation(
            target=base_predictor_record.to_predict, args=learn_args, integration_id=self.integration_id
        )
        ...
        predictor_record = db.Predictor(
            ...
            active=False,
            training_metadata={"hostname": socket.gethostname(), "reason": "finetune"},
        )
        db.serializable_insert(predictor_record)
