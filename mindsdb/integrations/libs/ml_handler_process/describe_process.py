...
def describe_process(
    integration_id: int, attribute: Optional[Union[str, list]], model_id: int, module_path: str
) -> DataFrame:
    ...
    if attribute is not None:
        ...
        try:
            ml_handler = module.Handler(engine_storage=handlerStorage, model_storage=modelStorage)
            return ml_handler.describe(attribute)
        except NotImplementedError:
            return DataFrame()
        except Exception as e:
            return DataFrame([f"{e.__class__.__name__}: {e}"], columns=["error"])
    else:
