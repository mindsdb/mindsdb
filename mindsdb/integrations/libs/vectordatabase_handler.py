...
class VectorStoreHandlerConfig(BaseModel):
    ...
    def check_param_typos(cls, values):
        ...
        for key in values.keys():
            if key not in expected_params:
                close_matches = difflib.get_close_matches(key, expected_params)
                if close_matches:
                    raise ValueError(
    ...
    def check_config(cls, values):
        ...
        url = values.get("url")
        persistence_directory = values.get("persistence_directory")
        ...
        if port and host and (url or persistence_directory):
            raise ValueError(
                f"For {vector_store} handler - if host and port are provided, "
                f"url and persistence_folder should not be provided."
            )

        if url and (host or port or persistence_directory):
            raise ValueError(
                f"For {vector_store} handler - if url is provided, host, port, "
                f"persistence_directory should not be provided."
            )

        if persistence_directory and (url or host or port):
            raise ValueError(
...
class VectorStoreHandler(BaseHandler):
    ...
    def __init__(self, name: str, **kwargs):
        ...
        if self.config.persistence_folder:
            # get full persistence directory from handler storage
            self.persistence_directory = self.handler_storage.folder_get(
                self.config.persistence_folder
            )
    ...
    def __del__(self):
        if self.is_connected is True:
            if self.persistence_directory:
                ...
                self.handler_storage.folder_sync(self.persistence_directory)
