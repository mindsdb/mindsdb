...
def validate_dataset(ml_task_type=None, dataset_name=None):
    if ml_task_type is None:
        raise MLTaskTypeMissing("ML Task type is missing. Please provide a valid 'ml_task_type'.")

    if dataset_name is None:
        raise DatasetNameMissing("Dataset name is missing. Please provide a valid 'dataset_name'.")
    ...
    if not dataset_path.exists():
        raise DatasetNotFound(f"Dataset '{dataset_name}' for ML Task type '{ml_task_type}' not found '{dataset_path}'.")
...
def validate_dataframe(df, mandatory_columns):
    ...
    if not columns_exist:
        raise MissingColumns(f"Columns {mandatory_columns} are missing from the dataframe.")
