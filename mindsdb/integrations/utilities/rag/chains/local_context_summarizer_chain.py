...
class LocalContextSummarizerChain(Chain):
    ...
    def _select_chunks_from_vector_store(self, doc_id: str) -> DataFrame:
        condition = FilterCondition(f"{self.metadata_column_name}->>'{self.doc_id_key}'", FilterOperator.EQUAL, doc_id)
        return self.vector_store_handler.select(
            self.table_name, columns=[self.content_column_name, self.metadata_column_name], conditions=[condition]
        )

    async def _get_all_chunks_for_document(self, doc_id: str) -> List[Document]:
        df = await asyncio.get_event_loop().run_in_executor(None, self._select_chunks_from_vector_store, doc_id)
        chunks = []
        ...
        for chunk in context_chunks:
            ...
            matching_chunk = next(
                (
                    c
                    for c in chunks
                    if c.metadata.get(self.doc_id_key) == doc_id
