...
class MapReduceSummarizerChain(Chain):
    ...
    def _select_chunks_from_vector_store(self, conditions: List[FilterCondition]) -> DataFrame:
        return self.vector_store_handler.select(
            self.table_name, columns=[self.content_column_name, self.metadata_column_name], conditions=conditions
        )
    ...
    async def _get_all_chunks_for_document(self, id: str) -> List[Document]:
        ...
        id_filter_condition = FilterCondition(
            f"{self.metadata_column_name}->>'{self.doc_id_key}'", FilterOperator.EQUAL, id
        )
        all_source_chunks = await asyncio.get_event_loop().run_in_executor(
            None, self._select_chunks_from_vector_store, [id_filter_condition]
        )
        document_chunks = []
    ...
    async def _get_source_summaries(
        self, source_ids: List[str], map_reduce_documents_chain: MapReduceDocumentsChain
    ) -> List[Summary]:
    ...
    def _call(self, inputs: Dict[str, Any], run_manager: Optional[CallbackManagerForChainRun] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            summaries = asyncio.get_event_loop().run_until_complete(
                self._get_source_summaries(unique_document_ids, map_reduce_documents_chain)
            )
        except RuntimeError:
