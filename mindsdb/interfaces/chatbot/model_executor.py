...
class ModelExecutor:
    def __init__(self, chat_task, model_name):
        ...
        model = chat_task.session.model_controller.get_model(model_name, project_name=chat_task.project_name)
    ...
    def call(self, history: List[ChatBotMessage], functions):
        ...
        if model_info["engine"] == "langchain":
            ...
            for function in functions:
                all_tools.append(
                    {"name": function.name, "func": function.callback, "description": function.description}
                )
            ...
            predictions = self.chat_task.project_datanode.predict(
                model_name=model_info["model_name"], df=pd.DataFrame(messages), params=params
            )
