...
class DatabaseController:
    ...
    def get_list(self, company_id: int, filter_type: str = None):
        ...
        result = [{'name': 'information_schema', 'type': 'system', 'id': None}]
        for x in projects:
            result.append({'name': x.name, 'type': 'project', 'id': x.id})
        for key, value in integrations.items():
            result.append({'name': key, 'type': 'integration', 'id': value.get('id')})
