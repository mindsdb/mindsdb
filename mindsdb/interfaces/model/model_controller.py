...
from mindsdb.interfaces.model.functions import get_model_record, get_model_records
...
class ModelController:
    ...
    def get_model_data(
        self, name: str = None, predictor_record=None, ml_handler_name="lightwood"
    ) -> dict:
        if predictor_record is None:
            predictor_record = get_model_record(
                except_absent=True, name=name, ml_handler_name=ml_handler_name
            )
    ...
    def get_reduced_model_data(
        ...
        if full_model_data.get("training_start_at") is not None:
            if full_model_data.get("training_stop_at") is not None:
                reduced_model_data["training_time"] = full_model_data.get(
                    "training_stop_at"
                ) - full_model_data.get("training_start_at")
            elif full_model_data.get("status") == "training":
                reduced_model_data[
                    "training_time"
                ] = dt.datetime.now() - full_model_data.get("training_start_at")
            if reduced_model_data["training_time"] is not None:
                reduced_model_data["training_time"] = reduced_model_data[
                    "training_time"
                ] - dt.timedelta(
                    microseconds=reduced_model_data["training_time"].microseconds
                )

        return reduced_model_data

    def describe_model(
        self, session, project_name, model_name, attribute, version=None
    ):
        ...
        ml_handler_base = session.integration_controller.get_handler(
    ...
    def delete_model(
        self, model_name: str, project_name: str = "mindsdb", version=None
    ):
        ...
        if is_cloud:
            for predictor_record in predictors_records:
                ...
                if (
                    ...
                    and (
                        dt.datetime.now() - parse_datetime(model_data.get("created_at"))
                    )
    ...
    def prepare_create_statement(
        self, statement, database_controller, handler_controller
    ):
        ...
        data_integration_ref, fetch_data_query = self._get_data_integration_ref(
            statement, database_controller
        )
        ...
        if statement.order_by is not None:
            ...
            for attr in ["horizon", "window"]:
                if getattr(statement, attr) is not None:
                    problem_definition["timeseries_settings"][attr] = getattr(
                        statement, attr
                    )

            if statement.group_by is not None:
                problem_definition["timeseries_settings"]["group_by"] = [
                    col.parts[-1] for col in statement.group_by
                ]
    ...
    def create_model(self, statement, ml_handler):
        params = self.prepare_create_statement(
            statement, ml_handler.database_controller, ml_handler.handler_controller
        )

        existing_projects_meta = ml_handler.database_controller.get_dict(
            filter_type="project"
        )
        if params["project_name"] not in existing_projects_meta:
            raise Exception(f"Project '{params['project_name']}' does not exist.")

        project = ml_handler.database_controller.get_project(
            name=params["project_name"]
        )
        project_tables = project.get_tables()
        if params["model_name"] in project_tables:
            raise Exception(
                f"Error: model '{params['model_name']}' already exists in project {params['project_name']}!"
            )
    ...
    def retrain_model(self, statement, ml_handler):
        ...
        params["version"] = self._get_retrain_finetune_version(
            params["project_name"], base_predictor_record
        )
    ...
    def _get_retrain_finetune_version(project_name, base_predictor_record):
        if base_predictor_record is None:
            raise Exception(
                f"Error: model '{base_predictor_record.name}' does not exist"
            )

        models = get_model_records(
            name=base_predictor_record.name, project_name=project_name, active=None
        )
    ...
    def prepare_finetune_statement(self, statement, database_controller):
        project_name, model_name, model_version, _describe = resolve_model_identifier(
            statement.name
        )
        data_integration_ref, fetch_data_query = self._get_data_integration_ref(
            statement, database_controller
        ...
        version = self._get_retrain_finetune_version(
            project_name, base_predictor_record
        )
    ...
    def finetune_model(self, statement, ml_handler):
        params = self.prepare_finetune_statement(
            statement, ml_handler.database_controller
        )
    ...
    def delete_model_version(self, models):
        ...
        for model in models:
            ...
            if model_record.active:
                raise Exception(
                    f"Can't remove active version: f{model['PROJECT']}.{model['NAME']}.{model['VERSION']}"
                )
