    ...
    def get_tools(self, prefix='') -> List[BaseTool]:
        ...
        list_sql_database_tool = ListSQLDatabaseTool(
            ...
            description=dedent("""
                ...
                Output: comma-separated list of tables in the database. Each table name is escaped using backticks.
                Description: Each table name in the list may be in one of two formats: database_name.`table_name` or database_name.schema_name.`table_name`.
                Table names in response to the user must be escaped using backticks.
            """)
        )

        info_sql_database_tool_description = dedent(f"""
            ...
            Description: Use this tool to investigate table schemas for needed columns.
            Ensure tables exist by calling {list_sql_database_tool.name} first.
        ...
        query_sql_database_tool_description = dedent(f"""
            Input: A detailed and well-structured SQL query in the MySQL dialect. The query must be enclosed between the symbols $START$ and $STOP$.
            ...
            6. Query Best Pract be escaped using backticks.tices:
        ...
        mindsdb_sql_parser_tool_description = dedent(f"""
            Input: A single SQL query
            Output: Valid parsed query, corrected query, or error message
            Description:
            Use this tool to ensure a SQL query passes the MindsDB SQL parser. ALWAYS use before {query_sql_database_tool.name}.
            If receiving an error message, rewrite and retry the query. If receiving a query, use the query in following steps.
        """)
