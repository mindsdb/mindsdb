...
class StreamIntegration(Integration):
    ...
    def _loop(self):
        ...
        while not STOP_THREADS_EVENT.wait(1.0):
                    ...
                    if "action" not in dct:
                        ...
                        if dct["action"] == "create":
                            for k in ["name", "predictor", "stream_in", "stream_out"]:
                                ...
                                logger.info(
                                    "INTEGRATION %s: creating stream %s",
                                    self.name,
                                    dct["name"],
                                )
                                if (
                                    db.session.query(db.Stream)
                                    .filter_by(
                                        name=dct["name"], company_id=self.company_id
                                    )
                                    .first()
                                    is None
                                ):
            ...
            stream_db_recs = (
                db.session.query(db.Stream)
                .filter_by(company_id=self.company_id, integration=self.name)
                .all()
            )
            ...
            for i, s in enumerate(self._streams):
                if s.name not in map(lambda x: x.name, stream_db_recs):
                    logger.info(
                        "INTEGRATION %s: stopping stream - %s", self.name, s.name
                    )
                    indices_to_delete.append(i)
                    self._streams[i].stop_event.set()
            self._streams = [
                s for i, s in enumerate(self._streams) if i not in indices_to_delete
            ]
            ...
            for s in stream_db_recs:
                if s.name not in map(lambda x: x.name, self._streams):
                    logger.info(
                        "INTEGRATION %s: starting stream - %s", self.name, s.name
                    )
