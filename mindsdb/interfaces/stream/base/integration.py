...
class StreamIntegration(Integration):
    ...
    def _loop(self):
        logger.info("INTEGRATION %s: starting", self.name)
        while not STOP_THREADS_EVENT.wait(1.0):
            if self._control_stream is not None:
                ...
                for dct in self._control_stream.read():
                    if 'action' not in dct:
                        logger.error('INTEGRATION %s: no action value found in control record - %s', self.name, dct)
                    else:
                        if dct['action'] == 'create':
                            for k in ['name', 'predictor', 'stream_in', 'stream_out']:
                                if k not in dct:
                                    ...
                                    logger.error('INTEGRATION %s: stream creating error. not enough data in control record - %s', self.name, dct)
                                    break
                            else:
                                logger.info('INTEGRATION %s: creating stream %s', self.name, dct['name'])
                                if db.session.query(db.Stream).filter_by(name=dct['name'], company_id=self.company_id).first() is None:
                                    ...
                                    logger.error('INTEGRATION %s: stream with this name already exists - %s', self.name, dct['name'])
                        elif dct['action'] == 'delete':
                            for k in ['name']:
                                if k not in dct:
                                    # Not all required parameters were provided (i.e. stream will not be created)
                                    # TODO: what's a good way to notify user about this?
                                    logger.error('INTEGRATION %s: unable to delete stream - stream name is not provided', self.name)
                                    break
                            else:
                                logger.error('INTEGRATION %s: deleting stream - %s', self.name, dct['name'])
                            ...
                            # Bad action value
                            logger.error('INTEGRATION %s: bad action value received - %s', self.name, dct)
            ...
            for i, s in enumerate(self._streams):
                if s.name not in map(lambda x: x.name, stream_db_recs):
                    logger.info("INTEGRATION %s: stopping stream - %s", self.name, s.name)
            ...
            for s in stream_db_recs:
                if s.name not in map(lambda x: x.name, self._streams):
                    logger.info("INTEGRATION %s: starting stream - %s", self.name, s.name)
                    self._streams.append(self._make_stream(s))

        logger.info("INTEGRATION %s: stopping", self.name)
