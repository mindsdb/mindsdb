        ...
        for ele in columns:
            ...
            for row_ind in indexes:
                if ludwig_dtype == 'order_by_col':
                    ts_data_point = self.transaction.input_data.data_frame[row_ind][col_ind]
                    ...
                    arr_str = self.transaction.input_data.data_frame[row_ind][col_ind]
                ...
                # Date isn't supported yet, so we hack around it
                    ...
                    date = parse_datetime(self.transaction.input_data.data_frame[row_ind][col_ind])
                    ...
                    if self.transaction.input_data.data_frame[row_ind][col_ind] is None:
                        unix_ts = 0
                    else:
                        unix_ts = parse_datetime(self.transaction.input_data.data_frame[row_ind][col_ind]).timestamp()

                    data[col].append(unix_ts)

                elif data_subtype in (DATA_SUBTYPES.FLOAT):
                    if type(self.transaction.input_data.data_frame[row_ind][col_ind]) == str:
                        data[col].append(float(str(self.transaction.input_data.data_frame[row_ind][col_ind]).replace(',','.')))
                    else:
                        data[col].append(self.transaction.input_data.data_frame[row_ind][col_ind])

                elif data_subtype in (DATA_SUBTYPES.INT):
                    if type(self.transaction.input_data.data_frame[row_ind][col_ind]) == str:
                        data[col].append(round(float(str(self.transaction.input_data.data_frame[row_ind][col_ind]).replace(',','.'))))
                    else:
                        data[col].append(self.transaction.input_data.data_frame[row_ind][col_ind])
