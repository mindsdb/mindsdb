...
class ModelAnalyzer(BaseModule):
    def run(self):
        ...
        if len(ignorable_input_columns) == 0:
            for pcol in output_columns:
                for i in range(len(self.transaction.input_data.validation_df[pcol])):
                    probabilistic_validators[pcol].register_observation(features_existence=[True for col in input_columns], real_value=self.transaction.input_data.validation_df[pcol].loc[i], predicted_value=normal_predictions[pcol][i], hmd=self.transaction.hmd)
        ...
        for column_name in ignorable_input_columns:
            ...
            for pcol in output_columns:
                for i in range(len(self.transaction.input_data.validation_df[pcol])):
                    probabilistic_validators[pcol].register_observation(features_existence=features_existence, real_value=self.transaction.input_data.validation_df[pcol].loc[i], predicted_value=ignore_col_predictions[pcol][i], hmd=self.transaction.hmd)
                    probabilistic_validators[pcol].register_observation(features_existence=[True for col in input_columns], real_value=self.transaction.input_data.validation_df[pcol].loc[i], predicted_value=normal_predictions[pcol][i], hmd=self.transaction.hmd)
