...
def upgrade():
    ...
    for row in predictors:
        ...
        record = db.JsonStorage(
            resource_group=RESOURCE_GROUP.PREDICTOR, resource_id=row["id"], name="json_ai", content=jai, company_id=None
        )
        session.add(record)
...
def downgrade():
    ...
    for row in jsons:
        predicrtor_record = session.query(db.Predictor).filter_by(company_id=None, id=row["resource_id"]).first()
