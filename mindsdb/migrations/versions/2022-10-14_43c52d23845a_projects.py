...
def upgrade():
    ...
    conn.execute(project_table.insert().values(name="mindsdb"))

    project_record = conn.execute(project_table.select().where(project_table.c.name == "mindsdb")).fetchone()
    ...
    for row in views:
        conn.execute(
            text("""
                ...
                where id = :view_id
            """),
            {"name": f"{row['name']}_view", "view_id": row["id"]},
        )
