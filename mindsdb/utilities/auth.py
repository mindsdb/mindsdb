...
def get_aws_meta_data() -> dict:
    ...
    aws_token = requests.put(
        "http://169.254.169.254/latest/api/token", headers={"X-aws-ec2-metadata-token-ttl-seconds": "30"}
    ).text
    for key in aws_meta_data.keys():
        resp = requests.get(
            f"http://169.254.169.254/latest/meta-data/{key}", headers={"X-aws-ec2-metadata-token": aws_token}, timeout=1
        )
...
def register_oauth_client():
    ...
    if current_aws_meta_data.get("public-hostname") != public_hostname or oauth_meta.get("client_id") is None:
        resp = requests.post(
        ...
        Config().update(
            {
                ...
                "auth": {"oauth": {"client_id": keys["client_id"], "client_secret": keys["client_secret"]}},
            }
        )
