    ...
    def prepare_env_config(self) -> None:
        ...
        if os.environ.get("MINDSDB_RERANKER_LOGPROBS", "") != "":
            ...
            if logprobs_value in ("true", "1", "yes", "y"):
                ...
                raise ValueError(f"MINDSDB_RERANKER_LOGPROBS must be a boolean value, got: {os.environ['MINDSDB_RERANKER_LOGPROBS']}")
        
        if os.environ.get("MINDSDB_RERANKER_TOP_LOGPROBS", "") != "":
            try:
                reranker_config["top_logprobs"] = int(os.environ["MINDSDB_RERANKER_TOP_LOGPROBS"])
            except ValueError:
                raise ValueError(f"MINDSDB_RERANKER_TOP_LOGPROBS must be an integer, got: {os.environ['MINDSDB_RERANKER_TOP_LOGPROBS']}")
        
        if os.environ.get("MINDSDB_RERANKER_MAX_TOKENS", "") != "":
            try:
                reranker_config["max_tokens"] = int(os.environ["MINDSDB_RERANKER_MAX_TOKENS"])
            except ValueError:
                raise ValueError(f"MINDSDB_RERANKER_MAX_TOKENS must be an integer, got: {os.environ['MINDSDB_RERANKER_MAX_TOKENS']}")
