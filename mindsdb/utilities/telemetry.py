...
def add_telemetry_record(event: str, record: dict) -> None:
    ...
    telemetry_records = db.session.query(db.Telemetry).filter_by(company_id=None).order_by(db.Telemetry.created_at).all()
...
def create_telemetry_start_record() -> None:
    ...
    integrations = db.session.query(db.Integration).filter_by(company_id=None).all()  # + deleted_at is None
    knowledge_bases = db.session.query(db.KnowledgeBase).all() # + deleted_at is None
    agents = db.session.query(db.Agents).filter_by(company_id=None).all()
    ...
    integrations = [record.engine for record in integrations if record.engine in data_handlers]
    integrations_dict = {item: integrations.count(item) for item in set(integrations)}
    ...
    for agent in agents:
        ...
        agents_list.append({
            k: v for k, v in model_params.items() if k in ("provider", "model_name")
        })
    ...
    add_telemetry_record(event, record)
