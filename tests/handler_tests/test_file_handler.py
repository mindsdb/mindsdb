...
TEST_PDF = TEST_DATA_PATH / "test.pdf"
TEST_TXT = TEST_DATA_PATH / "test.txt"
...
class FileHandlerTest(unittest.TestCase):
    ...
    def test_handle_txt(self):
        (df, _) = FileHandler._handle_source(TEST_TXT)
        assert isinstance(df, pandas.DataFrame)
        assert list(df.columns) == ["text"]
        assert df.shape == (6, 1)

    def test_handle_pdf(self):
        (df, _) = FileHandler._handle_source(TEST_PDF)
        assert isinstance(df, pandas.DataFrame)
        assert list(df.columns) == ["text"]
        assert df.shape == (27, 1)
