...
from mindsdb.integrations.handlers.gmail_handler.gmail_handler import (
    EmailsTable,
    GmailHandler,
)
...
class GmailHandlerTest(unittest.TestCase):
    ...
    @patch(
        "mindsdb.integrations.handlers.gmail_handler.gmail_handler.requests.get"
    )  # Patching the requests.get method
    ...
    def test_has_creds_file_with_invalid_s3_link(self, mock_get):
        ...
        with patch(
            "mindsdb.integrations.handlers.gmail_handler.gmail_handler.logger"
        ) as mock_logger:
            result = self.handler._has_creds_file(self.credentials_file)
            # Assert that the requests.get method was called with the correct URL
            ...
            mock_logger.error.assert_called_once_with(
                "Failed to get credentials from S3", 404
            )
    ...
    def test_parse_parts_with_multipart_mime_type_and_no_parts(self):
        email_parts = [{"mimeType": "multipart/mixed", "parts": []}]
        attachments = []
        email_body = self.handler._parse_parts(email_parts, attachments)
        expected_body = ""
        expected_attachments = []
        ...
        email_parts = [
            {
                ...
                "parts": [
                    ...
                    },
                    {
                        ...
                        "body": {"attachmentId": "<<attachment_id2>>"},
                    },
                ],
            }
        ]
...
class EmailsTableTest(unittest.TestCase):
    ...
    def test_delete_method(self):
        ...
        gmail_handler.call_gmail_api.assert_called_once_with(
            "delete_message", {"id": 1}
        )
    ...
    def test_update_method(self):
        ...
        gmail_table = EmailsTable(gmail_handler)
        query = parse_sql(
            'update gmail set addLabel="test1",removeLabel = "test" where id=1',
            dialect="mindsdb",
        )
        gmail_table.update(query)
        gmail_handler.call_gmail_api.assert_called_once_with(
            "modify_message",
            {"id": 1, "body": {"addLabelIds": ["test1"], "removeLabelIds": ["test"]}},
        )
