...
class EmailsTableTest(unittest.TestCase):
    ...
    def test_delete_method_with_normal_query(self):
        gmail_handler = Mock(GmailHandler)
        gmail_table = EmailsTable(gmail_handler)
        query = parse_sql('delete from gmail where message_id=1', dialect='mindsdb')
        gmail_table.delete(query)
        gmail_handler.call_gmail_api.assert_called_once_with('delete_message', {'id': 1})

    def test_delete_method_with_or_in_where_clause(self):
        gmail_handler = Mock(GmailHandler)
        gmail_table = EmailsTable(gmail_handler)
        with self.assertRaises(NotImplementedError):
            query = parse_sql('delete from gmail where message_id=1 or label_ids="test"', dialect='mindsdb')
            gmail_table.delete(query)

    def test_delete_method_with_invalid_variable_in_where_clause(self):
        gmail_handler = Mock(GmailHandler)
        gmail_table = EmailsTable(gmail_handler)
        with self.assertRaises(NotImplementedError):
