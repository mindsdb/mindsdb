...
def generate_test_cases_for_parametrization() -> List[Dict[str, Any]]:
    ...
    for handler in handlers_to_test:
        ...
        if skip_reason:
            test_cases.append({"handler_name": handler_name, "test_type": "skipped_test", "reason": skip_reason})
            continue
        ...
        try:
            ...
            create_query = (
                f"CREATE DATABASE {temp_db_name} WITH ENGINE = '{handler_name}', PARAMETERS = {params_clause};"
            )
            ...
            test_cases.append({"handler_name": handler_name, "test_type": "skipped_test", "reason": error_message})
            continue
        finally:
            server.query(f"DROP DATABASE IF EXISTS {temp_db_name};").fetch()
...
def test_handler_integrations(mindsdb_server, session_databases, test_case):
    ...
    if test_type == "negative":
        ...
        assert details["expected_error"] in error_str, (
            f'Negative test failed. Expected error substring "{details["expected_error"]}" not found in actual error: {error_str}'
        )
    else:  # Positive tests
