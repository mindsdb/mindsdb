...
def app():
    ...
    with TemporaryDirectory(prefix="knowledge_bases_integration_test_") as temp_dir:
        ...
        create_ml_engine_data = {"query": create_ml_engine_query}
...
def pgvector_database_name(client):
    ...
    conn_info = CONNECTION_KWARGS["connection_data"].copy()
    conn_info["database"] = new_db_name
    example_db_data = {"database": {"name": new_db_name, "engine": "pgvector", "parameters": conn_info}}
...
@pytest.mark.skipif(
    os.environ.get("MDB_TEST_PGVECTOR_HOST") is None, reason="MDB_TEST_PGVECTOR_HOST environment variable not set"
)
...
class TestKnowledgeBaseCompletion:
    def test_chat_completion(self, client, pgvector_database_name):
        ...
        create_request = {
            "knowledge_base": {
                ...
                "storage": {"table": TEST_TABLE_NAME, "database": pgvector_database_name},
            }
        }
        create_kb_response = client.post(
            "/api/projects/mindsdb/knowledge_bases", json=create_request, follow_redirects=True
        )
        assert create_kb_response.status_code == HTTPStatus.CREATED

        # Insert documents to help answer the question.
        rows_to_insert = [{"content": "The capital of Tyler Fantasy RAG Land is MindsDB"}]
        update_request = {"knowledge_base": {"rows": rows_to_insert}}
        update_kb_response = client.put(
            f"/api/projects/mindsdb/knowledge_bases/{test_kb_name}", json=update_request, follow_redirects=True
        )
        assert update_kb_response.status_code == HTTPStatus.OK

        completion_request = {"query": "What is the capital of Tyler Fantasy RAG Land?", "llm_model": "gpt-4o"}
        response = client.post(
            f"/api/projects/mindsdb/knowledge_bases/{test_kb_name}/completions",
    ...
    def test_context_completion_with_keywords(self, client, pgvector_database_name):
        ...
        completion_request = {"query": "Population of rag land", "keywords": "population rag land", "type": "context"}
        response = client.post(
            f"/api/projects/mindsdb/knowledge_bases/{test_kb_name}/completions",
            json=completion_request,
            follow_redirects=True,
    ...
    def test_context_completion_with_metadata(self, client, pgvector_database_name):
        ...
        completion_request = {"query": "Population of rag land", "metadata": {"author": "Tyler"}, "type": "context"}
        response = client.post(
            f"/api/projects/mindsdb/knowledge_bases/{test_kb_name}/completions",
            json=completion_request,
            follow_redirects=True,
