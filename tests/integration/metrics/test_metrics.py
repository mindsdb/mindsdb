...
class TestMetrics(HTTPHelperMixin):
    @pytest.mark.skipif(
        ("localhost" in HTTP_API_ROOT or "127.0.0.1" in HTTP_API_ROOT)
        and os.getenv("PROMETHEUS_MULTIPROC_DIR") is None,
        reason="PROMETHEUS_MULTIPROC_DIR environment variable is not set",
    )
    def test_http_metrics(self):
        # Make an HTTP request and check for updated metrics.
        api_metric_labels = {"endpoint": "/util/ping_native", "method": "GET", "status": "200"}
