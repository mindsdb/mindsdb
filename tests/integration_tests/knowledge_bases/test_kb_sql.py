...
class Test:
    ...
    def check_syntax_create_update_drop(self):
        ...
        '''
        self.run_sql('drop knowledge base if exists kb_crm')

        self.run_sql(f'''
            create knowledge base kb_crm
        ...
        # clean
        try:
            self.run_sql('delete from kb_crm where id in (select id from kb_crm)')
        except:
            ...
        ...
        print('Limit')
        ret = self.run_sql(f"select id, chunk_content from kb_crm limit 4")
        assert len(ret) == 4

        print('Limit with content')
        ret = self.run_sql(f"select id, chunk_content from kb_crm where content = 'help' limit 4")
        assert len(ret) == 4
        assert ret['id'][0] == '1000'  # id is string

        print('filter by id')
        ret = self.run_sql(f"select id, chunk_content from kb_crm where id = '1001'")
        assert len(ret) == 1
        assert ret['chunk_content'][0] == 'Thank you'

        ret = self.run_sql(f"select id, chunk_content from kb_crm where id != '1000' limit 4")
        assert len(ret) == 4
        assert '1000' not in ret['id']

        # in, not in
        ret = self.run_sql(f"select id, chunk_content from kb_crm where id in ('1001', '1000')")
        assert len(ret) == 2
        assert set(ret['id']) == {'1000', '1001'}

        ret = self.run_sql(f"select id, chunk_content from kb_crm where id not in ('1001', '1000') limit 4")
        assert len(ret) == 4
        assert '1000' not in ret['id']
        ...
        if self.vectordb_engine != 'chromadb':
            ...
            # like / not like
            ret = self.run_sql(f"select id, chunk_content from kb_crm where id like '100%')")
            assert len(ret) == 2
            assert '1001' in ret['id']

            ret = self.run_sql(f"select id, chunk_content from kb_crm where id not like '100%' limit 4")
            assert len(ret) == 4
        ...
        # delete
        self.run_sql(f"delete from kb_crm where id = '1'")
        ret = self.run_sql(f"select * from kb_crm where id = '1'")
        assert len(ret) == 0

        self.run_sql(f"delete from kb_crm where id in ('1001', '2')")
        ret = self.run_sql(f"select * from kb_crm where id in ('1001', '2')")
        assert len(ret) == 0

        self.run_sql(f"delete from kb_crm where id in ('1001', '1')")

        # insert
        self.run_sql(f"""
            insert into kb_crm (id, content) values (2000, 'OK')           
        """)
        ret = self.run_sql(f"select * from kb_crm where id = '2000'")
        assert len(ret) == 1
    ...
    def check_ingestion(self):
        ...
        Scenario
            ...
            Add threads, compare speed
    
        Second test
            load kb using mindsdb jobs
