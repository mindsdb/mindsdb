...
def test_get_table(client):
    ...
    expected_table = {"name": "models", "type": "data"}
...
def test_create_table_no_name_aborts(client):
    table_data = {"table": {"select": "SELECT * FROM example_db.house_sales limit 10", "replace": True}}
    response = client.post(f"/api/databases/{TEST_DB_NAME}/tables", json=table_data, follow_redirects=True)
    assert response.status_code == HTTPStatus.BAD_REQUEST


def test_create_table_no_select_aborts(client):
    table_data = {"table": {"name": "test_create_table_house_sales", "replace": True}}
    response = client.post(f"/api/databases/{TEST_DB_NAME}/tables", json=table_data, follow_redirects=True)
    assert response.status_code == HTTPStatus.BAD_REQUEST
...
def test_create_table_already_exists_aborts(client):
    ...
    # Clean up.
    client.delete(
        f"/api/databases/{TEST_DB_NAME}/tables/test_create_table_already_exists_aborts", follow_redirects=True
    )


def test_create_table_invalid_select_aborts(client):
    table_data = {
        "table": {"name": "test_create_table_house_sales", "select": "SELECT AN ERROR AWWW YEAH", "replace": True}
    }
    response = client.post(f"/api/databases/{TEST_DB_NAME}/tables", json=table_data, follow_redirects=True)
    assert response.status_code == HTTPStatus.BAD_REQUEST
