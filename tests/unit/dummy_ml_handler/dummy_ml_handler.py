...
class DummyHandler(BaseMLEngine):
    ...
    def _get_model_verison(self):
        return self.model_storage._get_model_record(self.model_storage.predictor_id).version

    def describe(self, attribute=None):
        if attribute == "info":
            return pd.DataFrame([["dummy", self._get_model_verison()]], columns=["type", "version"])
        elif isinstance(attribute, list):
            return pd.DataFrame([[".".join(attribute), self._get_model_verison()]], columns=["attribute", "version"])
        else:
            tables = ["info"]
