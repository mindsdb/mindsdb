...
class Test(BaseExecutorMockPredictor):
    ...
    def test_ts_predictor_file(self):
        ...
        fd, file_path = tempfile.mkstemp(prefix="file_")
        os.close(fd)
        ...
        predict_result = [
            # window
            {"a": 2, "t": np.float64(2022.0), "g": "x", "__mindsdb_row_id": 2},
            {"a": 3, "t": np.float64(2023.0), "g": "x", "__mindsdb_row_id": 3},
            # horizon
            {"a": 1, "t": np.float64(2024.0), "g": "x", "__mindsdb_row_id": None},
            {"a": 1, "t": np.float64(2024.0), "g": "x", "__mindsdb_row_id": None},
            {"a": 1, "t": np.float64(2025.0), "g": "x", "__mindsdb_row_id": None},
        ]
        predict_result = pd.DataFrame(predict_result)
        self.mock_predict.side_effect = lambda *a, **b: predict_result
...
class TestComplexQueries(BaseExecutorMockPredictor):
    ...
    def test_union(self, mock_handler):
        ...
        ret = self.execute(sql.format(union=""))
        ret_df = self.ret_to_df(ret)
        assert list(ret_df.columns) == ["a", "b", "c"]
        assert ret_df.shape[0] == 4
    ...
    def test_update_from_select(self, mock_handler):
        ...
        # second is update
        assert (
            mock_handler().query.call_args_list[1][0][0].to_string()
            == "update table2 set a1=1, c1='ccc' where a1 = 1 AND b1 = 'ccc'"
        )
    ...
    def test_update_in_integration(self, mock_handler):
        ...
        # second is update
        assert (
            mock_handler().query.call_args_list[0][0][0].to_string()
            == "update table2 set a1=1, c1='ccc' where b1 = 'b'"
        )
