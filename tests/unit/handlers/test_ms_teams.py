...
try:
    ...
    pytestmark = pytest.mark.skip("MSTeams handler not installed")
...
class TestMSTeamsHandler(BaseHandlerTestSetup, unittest.TestCase):
    ...
    def create_handler(self):
        ...
        return MSTeamsHandler("teams", connection_data=self.dummy_connection_data, handler_storage=mock_handler_storage)
    ...
    def test_connect_with_valid_code_returns_access_token(self, mock_token_cache):
        ...
        mock_msal.acquire_token_by_authorization_code.return_value = {"access_token": "mock_access_token"}
    ...
    def test_connect_with_invalid_code_raises_error(self):
        ...
        with self.assertRaises(
            AuthException,
            msg="Error getting access token: AADSTS70000: The provided authorization code is invalid or has expired.",
        ):
            self.handler.connect()
    ...
    def test_connect_with_cache_returns_access_code(self, mock_token_cache):
        ...
        mock_msal = MagicMock()
        mock_msal.get_accounts.return_value = [{"mock_key": "mock_value"}]

        mock_msal.acquire_token_silent.return_value = {"access_token": "mock_access_token"}
...
class TestMSTeamsTeamsTable(MSTeamsResourceTestSetup, unittest.TestCase):
    ...
    def test_list(self, mock_get):
        ...
        mock_response.json.return_value = {"value": [mock_team]}
        mock_get.return_value = mock_response
...
class TestMSTeamsChannelsTable(MSTeamsResourceTestSetup, unittest.TestCase):
    ...
    def test_list_all(self, mock_get):
        ...
        mock_response.json.return_value = {"value": [mock_channel]}
        mock_get.return_value = mock_response
    ...
    def test_list_with_team_id(self, mock_get):
        ...
        response = self.resource.list(
            conditions=[FilterCondition(column="teamId", op=FilterOperator.EQUAL, value="mock_team_id")]
        )
    ...
    def test_list_with_channel_id(self, mock_get):
        ...
        mock_team = self.generate_mock_data(columns=TeamsTable(self.handler).get_columns())
        mock_channel = self.generate_mock_data()
        mock_response_1.json.return_value = {"value": [mock_team]}
        mock_response_2.json.return_value = {"value": [mock_channel]}

        mock_get.side_effect = [mock_response_1, mock_response_2]

        response = self.resource.list(
            conditions=[FilterCondition(column="id", op=FilterOperator.EQUAL, value="mock_id")]
        )
    ...
    def test_list_with_team_id_and_channel_id(self, mock_get):
        ...
        response = self.resource.list(
            conditions=[
                FilterCondition(column="teamId", op=FilterOperator.EQUAL, value="mock_team_id"),
...
class TestMSTeamsChannelMessagesTable(MSTeamsResourceTestSetup, unittest.TestCase):
    ...
    def test_list_without_team_id_and_channel_id_raises_error(self):
        ...
        """
        with self.assertRaises(
            ValueError, msg="The 'channelIdentity_teamId' and 'channelIdentity_channelId' columns are required."
        ):
    ...
    def test_list_with_team_id_and_channel_id(self, mock_get):
        ...
        mock_response.json.return_value = {"value": [mock_message]}
        mock_get.return_value = mock_response

        response = self.resource.list(
            conditions=[
                FilterCondition(column="channelIdentity_teamId", op=FilterOperator.EQUAL, value="mock_team_id"),
                FilterCondition(column="channelIdentity_channelId", op=FilterOperator.EQUAL, value="mock_channel_id"),
            ]
        )
...
class TestMSTeamsChatMessagesTable(MSTeamsResourceTestSetup, unittest.TestCase):
    ...
    def test_list_with_chat_id(self, mock_get):
        ...
        response = self.resource.list(
            conditions=[FilterCondition(column="chatId", op=FilterOperator.EQUAL, value="mock_chat_id")]
        )
    ...
    def test_list_with_chat_id_and_message_id(self, mock_get):
        ...
        response = self.resource.list(
            conditions=[
                FilterCondition(column="chatId", op=FilterOperator.EQUAL, value="mock_chat_id"),
                FilterCondition(column="id", op=FilterOperator.EQUAL, value="mock_id"),
            ]
        )
