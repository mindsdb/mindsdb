...
class TestOpenAI(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.handler = OpenAIHandler(
            mock_model_storage, mock_engine_storage, connection_data={"connection_data": self.dummy_connection_data}
        )
    ...
    def test_create_validation_without_using_clause_raises_exception(self):
        ...
        with self.assertRaisesRegex(
            Exception, "OpenAI engine requires a USING clause! Refer to its documentation for more details."
        ):
    ...
    def test_create_validation_without_required_parameters_raises_exception(self):
        ...
        with self.assertRaisesRegex(
            Exception, "One of `question_column`, `prompt_template` or `prompt` is required for this engine."
        ):
    ...
    def test_create_validation_with_invalid_parameter_combinations_raises_exception(self):
        ...
        with self.assertRaisesRegex(Exception, "^Please provide one of"):
            self.handler.create_validation(
                "target",
                args={"using": {"prompt_template": "dummy_prompt_template", "question_column": "question"}},
    ...
    def test_create_validation_with_unknown_arguments_raises_exception(self):
        ...
        with self.assertRaisesRegex(Exception, "^Unknown arguments:"):
            self.handler.create_validation(
                "target",
                args={"using": {"prompt_template": "dummy_prompt_template", "unknown_arg": "unknown_arg"}},
                handler_storage=self.mock_engine_storage,
            )
    ...
    def test_create_validation_with_invalid_api_key_raises_exception(self):
        ...
        with self.assertRaisesRegex(Exception, "Invalid api key"):
            self.handler.create_validation(
                "target",
                args={"using": {"prompt_template": "dummy_prompt_template"}},
                handler_storage=self.mock_engine_storage,
            )
    ...
    def test_create_with_invalid_mode_raises_exception(self, mock_openai_handler_openai_client):
        ...
        with self.assertRaisesRegex(Exception, "^Invalid operation mode."):
            self.handler.create(
                "dummy_target", args={"using": {"prompt_template": "dummy_prompt_template", "mode": "dummy_mode"}}
            )
    ...
    def test_create_with_unsupported_model_raises_exception(self, mock_openai_handler_openai_client):
        ...
        with self.assertRaisesRegex(Exception, "^Invalid model name."):
            self.handler.create(
                "dummy_target",
                args={
                    "using": {"model_name": "dummy_unsupported_model_name", "prompt_template": "dummy_prompt_template"}
                },
            )
    ...
    def test_predict_in_default_mode_without_question_column_in_data_raises_exception(self):
        ...
        # Mock the json_get method of the model storage
        self.handler.model_storage.json_get.return_value = {"mode": "default", "question_column": "question"}
    ...
    @patch("mindsdb.integrations.handlers.openai_handler.openai_handler.OpenAI")
    def test_predict_in_default_mode_with_question_column_using_valid_arguments_and_data_runs_no_errors(
    ...
    @patch("mindsdb.integrations.handlers.openai_handler.openai_handler.OpenAI")
    def test_predict_in_default_mode_with_prompt_template_using_valid_arguments_and_data_runs_no_errors(
        self, mock_openai_handler_openai_client
    ):
        """
    ...
    @patch("mindsdb.integrations.handlers.openai_handler.openai_handler.OpenAI")
    def test_predict_in_default_mode_with_question_column_and_completion_model_using_valid_arguments_and_data_runs_no_errors(
        self, mock_openai_handler_openai_client
    ):
        """
    ...
    @patch("mindsdb.integrations.handlers.openai_handler.openai_handler.OpenAI")
    def test_predict_in_default_mode_with_prompt_template_and_completion_model_using_valid_arguments_and_data_runs_no_errors(
        self, mock_openai_handler_openai_client
    ):
        """
    ...
    @patch("mindsdb.integrations.handlers.openai_handler.openai_handler.OpenAI")
    def test_predict_in_embedding_mode_using_valid_arguments_and_data_runs_no_errors(
        self, mock_openai_handler_openai_client
    ):
        """
        ...
        mock_openai_client = MagicMock()
        mock_openai_client.embeddings.create.return_value = MagicMock(data=[MagicMock(embedding=[0, 1])])
    ...
    @patch("mindsdb.integrations.handlers.openai_handler.openai_handler.OpenAI")
    def test_predict_in_image_mode_with_question_column_using_valid_arguments_and_data_runs_no_errors(
        self, mock_openai_handler_openai_client
    ):
        """
        ...
        mock_openai_client = MagicMock()
        mock_openai_client.images.generate.return_value = MagicMock(data=[MagicMock(url="dummy_image_url")])
    ...
    @patch("mindsdb.integrations.handlers.openai_handler.openai_handler.OpenAI")
    def test_predict_in_image_mode_with_prompt_template_using_valid_arguments_and_data_runs_no_errors(
        self, mock_openai_handler_openai_client
    ):
        """
    ...
    @patch("mindsdb.integrations.handlers.openai_handler.openai_handler.OpenAI")
    def test_predict_in_conversational_mode_with_using_arguments_and_data_runs_no_errors(
        self, mock_openai_handler_openai_client
    ):
        """
        ...
        mock_openai_client = MagicMock()
        mock_openai_client.chat.completions.create.return_value = MagicMock(
            choices=[MagicMock(message=MagicMock(content="Gamla Stan"))]
        )
    ...
    @patch("mindsdb.integrations.handlers.openai_handler.openai_handler.OpenAI")
    def test_predict_in_conversational_full_mode_using_valid_arguments_and_data_runs_no_errors(
        self, mock_openai_handler_openai_client
    ):
        """
        ...
        mock_openai_client.chat.completions.create.side_effect = [
            MagicMock(choices=[MagicMock(message=MagicMock(content="Stockholm"))]),
            MagicMock(choices=[MagicMock(message=MagicMock(content="Gamla Stan"))]),
        ]
    ...
    def test_finetune_with_unsupported_model_raises_exception(self):
        ...
        self.handler.base_model_storage.json_get.return_value = {"model_name": "dummy_model_name"}
