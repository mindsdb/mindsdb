...
from mindsdb.api.executor.planner.steps import (
    FetchDataframeStep,
    CreateTableStep, SubSelectStep,
    UpdateToTable, DeleteStep
)
...
class TestPlanIntegrationSelect:
            ...
            steps=[
                FetchDataframeStep(
                    ...
                    query=Select(
                        ...
                        from_table=Identifier('tab'),
                        where=BinaryOperation(
                            '=', args=[Identifier('column1'), Identifier('column2')]
                        ),
                        limit=Constant(10),
                        offset=Constant(15),
                    ),
    ...
    def test_integration_select_plan_complex_path(self):
        query = Select(
            targets=[Identifier(parts=['int', 'tab', 'a column with spaces'])],
            from_table=Identifier('int.tab')
        )
        expected_plan = QueryPlan(
            integrations=['int'],
    ...
    def test_integration_select_column_alias(self):
        query = Select(
            targets=[Identifier('col1', alias=Identifier('column_alias'))],
            from_table=Identifier('int.tab')
        )
    ...
    def test_integration_select_subquery_in_target(self):
        query = Select(
            targets=[Identifier('column1'), Select(
    ...
    def test_update_table(self):
        ...
        expected_plan = QueryPlan(
            ...
            steps=[
                UpdateToTable(
                    dataframe=None, table=Identifier('integration1.direct_messages'),
