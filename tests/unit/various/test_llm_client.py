...
# ---------------------------------------------------------------------------
# Lightweight stub for the one mindsdb module-level import in llm_client.py:
#   from mindsdb.integrations.utilities.handler_utils import get_api_key
#
# handler_utils pulls in HandlerStorage → db → mind_castle (a private package),
# so we stub only the parts llm_client.py actually uses at import time.
# All other imports in llm_client.py (openai, stdlib) resolve normally.
# ---------------------------------------------------------------------------
_handler_utils_stub = types.ModuleType("mindsdb.integrations.utilities.handler_utils")
_handler_utils_stub.get_api_key = MagicMock(return_value=None)

sys.modules.setdefault("mindsdb.integrations.utilities.handler_utils", _handler_utils_stub)

# Now the real module can be imported cleanly.
from mindsdb.interfaces.knowledge_base.llm_client import LLMClient  # noqa: E402
...
class TestLLMClientOllama(unittest.TestCase):
    ...
    def test_ollama_only_keys_not_forwarded_to_openai(self):
        ...
        disallowed = {
            "temperature", "top_p", "top_k", "timeout", "format", "headers",
            "num_predict", "num_ctx", "num_gpu", "repeat_penalty", "stop",
            "template", "api_keys", "provider", "model", "model_name",
        }
